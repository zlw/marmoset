...
STDLIB_SHIMS_SUITE="$INTEGRATION_DIR/14_stdlib_shims.sh"
...
Selectors:
  ...
  stdlib               # run 14_stdlib_shims.sh suite
  stdlib-shims         # run 14_stdlib_shims.sh suite
...
resolve_selector() {
    ...
    if [ "$name" = "all" ]; then
        echo "${ALL_GROUPS[*]} __SNAPSHOTS__ __HARDENING__ __PRELUDE__ __FFI__ __STDLIB_SHIMS__"
        return 0
    fi
    ...
    if [ "$name" = "snapshots" ] || [ "$name" = "codegen-snapshots" ] || [ "$name" = "10_codegen_snapshots.sh" ]; then
        echo "__SNAPSHOTS__"
        return 0
    fi
    ...
    if [ "$name" = "stdlib" ] || [ "$name" = "stdlib-shims" ] || [ "$name" = "14_stdlib_shims" ] || [ "$name" = "14_stdlib_shims.sh" ]; then
        echo "__STDLIB_SHIMS__"
        return 0
    fi
    ...
    for arg in "$@"; do
        ...
        for token in $resolved; do
            if [ "$token" = "__CLI__" ]; then
                ...
                run_ffi=1
            elif [ "$token" = "__STDLIB_SHIMS__" ]; then
                run_stdlib_shims=1
            else
...
if [ "${#unique_groups[@]}" -eq 0 ] && [ "${#selected_fixture_files[@]}" -eq 0 ] && [ "$run_cli" -eq 0 ] && [ "$run_harness" -eq 0 ] && [ "$run_snapshots" -eq 0 ] && [ "$run_hardening" -eq 0 ] && [ "$run_prelude" -eq 0 ] && [ "$run_ffi" -eq 0 ] && [ "$run_stdlib_shims" -eq 0 ]; then
...
if [ "$run_snapshots" -eq 1 ]; then
    suite_count=$((suite_count + 1))
    echo ""
    if "$SNAPSHOT_SUITE"; then
        suite_pass=$((suite_pass + 1))
    else
        suite_fail=$((suite_fail + 1))
    fi
...
if [ "$run_stdlib_shims" -eq 1 ]; then
    suite_count=$((suite_count + 1))
    echo ""
    if "$STDLIB_SHIMS_SUITE"; then
        suite_pass=$((suite_pass + 1))
    else
        suite_fail=$((suite_fail + 1))
    fi
