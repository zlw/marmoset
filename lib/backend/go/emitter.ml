...
module Type_narrowing = Typecheck.Type_narrowing
...
let rec mangle_type (t : Types.mono_type) : string =
  ...
  | Types.TUnion [] -> "union_empty"
  | Types.TUnion members -> "union_" ^ String.concat "_" (List.map mangle_type members)
...
let rec mangle_type_for_shape (t : Types.mono_type) : string =
  ...
  | Types.TUnion [] -> "union_empty"
  | Types.TUnion members -> "union_" ^ String.concat "_" (List.map mangle_type_for_shape members)
...
let ordering_go_type_name = "Ordering"
let ordering_less_tag = "Ordering_Less_tag"
let ordering_equal_tag = "Ordering_Equal_tag"
let ordering_greater_tag = "Ordering_Greater_tag"
let ordering_less_ctor = "Ordering_Less()"
let ordering_equal_ctor = "Ordering_Equal()"
let ordering_greater_ctor = "Ordering_Greater()"
...
let rec normalize_instantiation_type ~(concrete_only : bool) (t : Types.mono_type) : Types.mono_type =
  match normalize_type_for_codegen ~concrete_only t with
  | Types.TFun (arg, ret, eff) ->
      Types.TFun
        (normalize_instantiation_type ~concrete_only arg, normalize_instantiation_type ~concrete_only ret, eff)
  | Types.TArray elem -> Types.TArray (normalize_instantiation_type ~concrete_only elem)
  | Types.THash (key, value) ->
      Types.THash
        (normalize_instantiation_type ~concrete_only key, normalize_instantiation_type ~concrete_only value)
  | Types.TRecord (fields, row) ->
      let normalized_fields =
        List.map
          (fun (field : Types.record_field_type) ->
            { field with typ = normalize_instantiation_type ~concrete_only field.typ })
          fields
      in
      let normalized_row = Option.map (normalize_instantiation_type ~concrete_only) row in
      let normalized_fields, normalized_row = normalize_record_row_type normalized_fields normalized_row in
      Types.TRecord (normalized_fields, normalized_row)
  | Types.TUnion members -> Types.TUnion (List.map (normalize_instantiation_type ~concrete_only) members)
  | Types.TIntersection members ->
...
let canonical_enum_name_for_codegen (enum_name : string) : string =
  match Typecheck.Annotation.lookup_enum_by_source_name enum_name with
  | Some enum_def -> enum_def.name
  | None -> enum_name

let canonical_variant_name_for_codegen (enum_name : string) (variant_name : string) : string =
  let enum_name = canonical_enum_name_for_codegen enum_name in
...
let mangle_func_name_with_return name (param_types : Types.mono_type list) (return_type : Types.mono_type) :
    string =
  mangle_func_name name param_types ^ "_ret_" ^ mangle_type return_type
...
let path_name_fragment (path : string) : string = go_safe_ident (encoded_path_fragment path)
...
let dynamic_trait_methods_for (traits : string list) : dyn_callable_method list =
  let by_method_name :
      (string, (string * Typecheck.Trait_registry.trait_def * Typecheck.Trait_registry.method_sig) list) Hashtbl.t
      =
    Hashtbl.create 16
  in
  List.iter
...
type func_def = {
  ...
  signature_params : (string * AST.type_expr option) list;
        ...
        if c <> 0 then
          ...
          if c <> 0 then
            ...
            if c <> 0 then
              ...
              let c = String.compare a.type_fingerprint b.type_fingerprint in
              if c <> 0 then
                c
              else
...
let same_instantiation_param_key (left : instantiation) (right : instantiation) : bool =
  String.equal left.module_path right.module_path
  && left.func_expr_id = right.func_expr_id
  && String.equal left.func_name right.func_name
  && left.func_arity = right.func_arity
  && left.concrete_only_mode = right.concrete_only_mode
  && String.equal left.type_fingerprint right.type_fingerprint

let instantiation_needs_return_mangle (instantiations : InstSet.t) (inst : instantiation) : bool =
  InstSet.exists
    (fun (candidate : instantiation) ->
      same_instantiation_param_key candidate inst && candidate.return_type <> inst.return_type)
    instantiations

let mangle_instantiated_func_name (instantiations : InstSet.t) (inst : instantiation) : string =
  if instantiation_needs_return_mangle instantiations inst then
    mangle_func_name_with_return inst.func_name inst.concrete_types inst.return_type
  else
    mangle_func_name inst.func_name inst.concrete_types
...
module DerivedImplUseSet = Set.Make (struct
  type t = string * Types.mono_type

  let compare = compare
end)

module BuiltinImplUseSet = Set.Make (struct
...
type mono_state = {
  ...
  mutable derived_impl_uses : DerivedImplUseSet.t;
...
let is_std_bytes_type_name (name : string) : bool = String.equal name Typecheck.Shim_boundary.std_bytes_type_name
...
let overlay_method_type_args_map
    (base : (int, Types.mono_type list) Hashtbl.t) (overlay : (int, Types.mono_type list) Hashtbl.t) :
...
let apply_substitution_method_type_args_map
    (subst : Types.substitution) (method_type_args_map : (int, Types.mono_type list) Hashtbl.t) :
    (int, Types.mono_type list) Hashtbl.t =
...
let used_emitted_function_names (state : mono_state) : StringSet.t =
  let names =
    InstSet.fold
      (fun (inst : instantiation) acc ->
        StringSet.add (mangle_instantiated_func_name state.instantiations inst) acc)
      state.instantiations StringSet.empty
  in
  Hashtbl.fold (fun _ name acc -> StringSet.add name acc) state.extern_wrapper_names names

let extern_wrapper_name_for_state (state : mono_state) (func : Typecheck.Resolution_artifacts.extern_func) :
    string =
      ...
      let rec pick n =
        let candidate =
          if n = 0 then
            ...
            Printf.sprintf "%s__ffi%d" base n
        in
...
let register_builtin_impl_use
  ...
  in
  if not (state.concrete_only && not (Types.TypeVarSet.is_empty (Types.free_type_vars normalized_for_type))) then
    let type_suffix = mangle_type normalized_for_type in
...
let register_inherent_call_use
  ...
  if
    not
      (state.concrete_only
      && (not allow_erased_unresolved)
      && (has_free_type_vars receiver_type || List.exists has_free_type_vars method_type_args))
  then
    let receiver_type = normalize_type_for_codegen ~concrete_only:state.concrete_only receiver_type in
    let method_type_args =
      List.map (normalize_type_for_codegen ~concrete_only:state.concrete_only) method_type_args
    in
    state.inherent_call_uses <-
      InherentCallUseSet.add (method_name, receiver_type, method_type_args) state.inherent_call_uses

let placeholder_rewritten_expr (rewrite_map : Infer.placeholder_rewrite_map) (expr : AST.expression) :
    AST.expression =
...
let synthetic_helper_suffix (expr_id : int) : string =
  if expr_id < 0 then
    "neg" ^ string_of_int (-expr_id)
  else
    string_of_int expr_id
...
let lookup_method_def_exn
  ...
  | None ->
      failwith
        (Printf.sprintf "Codegen error: missing typed method metadata for %s.%s (method id %d)" owner method_name
           method_id)
...
let error_variant_message (enum_name : string) (variant : Typecheck.Enum_registry.variant_def) : string =
  ...
  | None ->
      failwith
        (Printf.sprintf "Codegen error: error variant %s.%s is missing a canonical message" enum_name variant.name)
...
let error_context_expr
    (state : mono_state) ~(message : string) ~(file_id : string) ~(function_name : string) ~(position : int) :
    string =
  let helper = marmoset_runtime_helper state "NewErrorContext" in
  Printf.sprintf "%s(%S, %S, %S, int64(%d))" helper message (stable_error_file_id file_id) function_name position
...
let named_type_codegen_body_exn (type_name : string) (type_args : Types.mono_type list) :
  ...
  | Some def -> (
      ...
      | Type_registry.NamedExtern _ ->
          failwith (Printf.sprintf "Codegen error: extern type '%s' has no Marmoset representation" type_name))
...
let rec shape_field_type_to_go (state : mono_state) (t : Types.mono_type) : string =
  ...
  | Types.THash (key, value) ->
      "map[" ^ shape_field_type_to_go state key ^ "]" ^ shape_field_type_to_go state value
  | Types.TRecord (fields, row) ->
...
and extern_type_to_go (t : Types.mono_type) : string =
  ...
  | unsupported ->
      failwith
        (Printf.sprintf "Codegen error: unsupported extern type reached backend: %s" (Types.to_string unsupported))
        ...
        (fun _key traits acc ->
          ...
          let method_fields =
            ...
            |> List.map (fun (method_info : dyn_callable_method) ->
                   ...
                   let arg_types =
                     method_sig.method_params |> List.tl |> List.map snd |> List.map (type_to_go state)
                   in
      ...
      |> List.map (fun (source_type, traits) ->
             ...
             let method_defs =
               ...
               |> List.map (fun (method_info : dyn_callable_method) ->
                      ...
                      register_builtin_impl_use state ~trait_name:dispatch_trait_name
                        ~method_name:method_sig.method_name ~for_type:source_type;
                      let adapter_name =
...
and enum_payload_needs_boxing (state : mono_state) ?(visited_named = StringSet.empty) (t : Types.mono_type) : bool
    =
  ...
  | Types.TArray _ | Types.THash _ | Types.TRowVar _ | Types.TTraitObject _ | Types.TUnion _ ->
      false
      ...
      (fun idx (pos, field_type) ->
        ...
        let go_type =
          ...
          if boxed then
            ...
            base_go_type
        in
        (pos, field_type, { data_field_name = field_name; data_field_type = field_type; go_type; boxed }))
      all_position_types
  in
...
let rec captures_top_level_values_expr
  ...
  | AST.Try { tried; fallback; _ } ->
      merge_sets
        (captures_top_level_values_expr top_level_values bound tried
        :: (fallback
...
let hoisted_local_func_name (name : string) (expr_id : int) : string =
  Printf.sprintf "__local_%s_%d" (go_safe_ident name) expr_id
...
let add_func_def
    ...
    ~(return_type : AST.type_expr option)
    ~(body : AST.statement)
    ~(func_expr_id : int) : unit =
  let param_exprs = List.map (fun (pname, _) -> AST.mk_expr (AST.Identifier pname)) params in
  state.func_defs <-
    {
      name;
      params = param_exprs;
...
let func_def_requires_return_mangle (func_def : func_def) : bool =
  let generic_names =
    match func_def.generics with
    | None -> []
    | Some generics -> List.map (fun (generic : AST.generic_param) -> generic.name) generics
  in
  match (generic_names, func_def.return_type) with
  | [], _ | _, None -> false
  | _ -> (
      let generic_seed = List.map (fun name -> (name, Types.TVar name)) generic_names in
      let convert type_expr =
        match Annotation.type_expr_to_mono_type_with generic_seed type_expr with
        | Ok typ -> Some typ
        | Error _ -> None
      in
      let param_vars =
        func_def.signature_params
        |> List.filter_map (fun (_name, annot_opt) -> Option.bind annot_opt convert)
        |> List.fold_left
             (fun acc typ -> Types.TypeVarSet.union acc (Types.free_type_vars typ))
             Types.TypeVarSet.empty
      in
      match convert (Option.get func_def.return_type) with
      | None -> false
      | Some return_type ->
          let return_vars = Types.free_type_vars return_type in
          generic_names
          |> List.exists (fun name ->
                 Types.TypeVarSet.mem name return_vars && not (Types.TypeVarSet.mem name param_vars)))

let string_set_of_list (names : string list) : StringSet.t =
  List.fold_left (fun acc name -> StringSet.add name acc) StringSet.empty names

let rec collect_funcs_stmt
    ?(top_level = false) ?(available_bindings = StringSet.empty) (state : mono_state) (stmt : AST.statement) :
    StringSet.t =
  match stmt.stmt with
  | AST.Let let_binding -> (
      ...
      | AST.Function f ->
          ...
          let captures =
            captures_top_level_values_stmt available_bindings param_bindings f.body |> StringSet.elements
          in
          let () =
            if captures = [] then
              let target_name =
              ...
              in
              add_func_def state ~name:target_name ~generics:f.generics ~params:f.params
                ~return_type:f.return_type ~body:f.body ~func_expr_id:value_expr.id
          in
          ...
          if top_level then
            ...
            StringSet.add let_binding.name available_bindings
      | _ ->
          if top_level then
            state.top_level_value_bindings <- StringSet.add let_binding.name state.top_level_value_bindings;
          collect_funcs_expr ~available_bindings state let_binding.value;
          StringSet.add let_binding.name available_bindings)
  | AST.Return e ->
      collect_funcs_expr ~available_bindings state e;
      available_bindings
...
and collect_funcs_expr ?(available_bindings = StringSet.empty) (state : mono_state) (expr : AST.expression) : unit
  ...
  | AST.TypeCheck (e, _) -> collect_funcs_expr ~available_bindings state e
  | AST.If (cond, cons, alt) ->
      collect_funcs_expr ~available_bindings state cond;
      ignore (collect_funcs_stmt ~top_level:false ~available_bindings state cons);
      Option.iter (fun stmt -> ignore (collect_funcs_stmt ~top_level:false ~available_bindings state stmt)) alt
  | AST.Call (f, args) ->
      collect_funcs_expr ~available_bindings state f;
      List.iter (collect_funcs_expr ~available_bindings state) args
  | AST.Array elements -> List.iter (collect_funcs_expr ~available_bindings state) elements
  | AST.Hash pairs ->
      List.iter
        (fun (k, v) ->
          collect_funcs_expr ~available_bindings state k;
          collect_funcs_expr ~available_bindings state v)
        pairs
  | AST.Index (container, index) ->
      collect_funcs_expr ~available_bindings state container;
      collect_funcs_expr ~available_bindings state index
  | AST.Function f ->
      let param_bindings = string_set_of_list (List.map fst f.params) in
      let captures =
        captures_top_level_values_stmt available_bindings param_bindings f.body |> StringSet.elements
      in
      (match (f.origin, captures) with
      | AST.PlaceholderSection, [] ->
          add_func_def state ~name:(hoisted_section_func_name expr.id) ~generics:f.generics ~params:f.params
            ~return_type:f.return_type ~body:f.body ~func_expr_id:expr.id
      | _ -> ());
      let nested_available = StringSet.union available_bindings (string_set_of_list (List.map fst f.params)) in
      ignore (collect_funcs_stmt ~top_level:false ~available_bindings:nested_available state f.body)
  | AST.EnumConstructor (_, _, args) -> List.iter (collect_funcs_expr ~available_bindings state) args
  | AST.Match (scrutinee, arms) ->
      collect_funcs_expr ~available_bindings state scrutinee;
      List.iter (fun arm -> collect_funcs_expr ~available_bindings state arm.AST.body) arms
  | AST.Try { tried; fallback; _ } ->
  ...
  | AST.RecordLit (fields, spread) -> (
      List.iter
        (fun field ->
          ...
          | Some expr -> collect_funcs_expr ~available_bindings state expr
          | None -> ())
        fields;
      match spread with
      | Some expr -> collect_funcs_expr ~available_bindings state expr
      | None -> ())
  | AST.FieldAccess (expr, _field) -> collect_funcs_expr ~available_bindings state expr
  | AST.MethodCall { mc_receiver; mc_args; _ } ->
      collect_funcs_expr ~available_bindings state mc_receiver;
      List.iter (collect_funcs_expr ~available_bindings state) mc_args
  | AST.BlockExpr stmts ->
      ignore
        (List.fold_left
           (fun bindings stmt -> collect_funcs_stmt ~top_level:false ~available_bindings:bindings state stmt)
...
let enum_constructor_codegen_type
  ...
  let rec extract_enum_type = function
    | Types.TEnum (_, _) as t -> Some t
    | Types.TFun (_, ret, _) -> extract_enum_type ret
    | t -> (
        match Types.canonicalize_mono_type t with
        | Types.TEnum (_, _) as enum_t -> Some enum_t
        | Types.TFun (_, ret, _) -> extract_enum_type ret
        | _ -> None)
  in
  let candidate_type =
  ...
  in
  match extract_enum_type candidate_type with
...
let hoisted_section_target_name (state : mono_state) (expr : AST.expression) : string option =
  let target_name = hoisted_section_func_name expr.id in
  if is_user_func state target_name then
    Some target_name
  else
    None
...
let unique_instantiated_func_name (state : mono_state) (name : string) : string option =
  ...
  | [ inst ] when not (List.exists has_type_vars inst.concrete_types) ->
      Some (mangle_instantiated_func_name state.instantiations inst)
  | _ -> None
...
let matches_instantiated_param (param_type : Types.mono_type) (arg_type : Types.mono_type) : bool =
  ...
  | Types.TUnion members ->
      List.exists
        (fun member ->
          match Unify.unify (Types.canonicalize_mono_type member) (Types.canonicalize_mono_type arg_type) with
          | Ok _ -> true
          | Error _ -> false)
        members
  | _ -> (
...
let instantiated_func_name_for_args (state : mono_state) (name : string) (arg_types : Types.mono_type list) :
  ...
  in
  match candidates with
  | [ inst ] -> Some (mangle_instantiated_func_name state.instantiations inst)
  | _ -> None

let instantiated_func_for_args (state : mono_state) (name : string) (arg_types : Types.mono_type list) :
...
let instantiated_func_for_signature
    (state : mono_state) (name : string) (arg_types : Types.mono_type list) (return_type : Types.mono_type) :
    instantiation option =
  let return_type = Types.canonicalize_mono_type return_type in
  let candidates =
    InstSet.elements state.instantiations
    |> List.filter (fun (inst : instantiation) ->
           inst.func_name = name
           && List.length inst.concrete_types = List.length arg_types
           && List.for_all2 matches_instantiated_param inst.concrete_types arg_types
           && Types.canonicalize_mono_type inst.return_type = return_type)
  in
  match candidates with
  | [ inst ] -> Some inst
  | _ -> None
...
let func_def_generic_seed (func_def : func_def) : (string * Types.mono_type) list =
  match func_def.generics with
  | None -> []
  | Some generics ->
      List.map (fun (generic : AST.generic_param) -> (generic.name, Types.TVar generic.name)) generics

let func_def_declared_signature (func_def : func_def) : (Types.mono_type list * Types.mono_type) option =
  let convert = Annotation.type_expr_to_mono_type_with (func_def_generic_seed func_def) in
  let rec convert_params acc params =
    match params with
    | [] -> Some (List.rev acc)
    | (_name, Some param_type_expr) :: rest -> (
        match convert param_type_expr with
        | Ok param_type -> convert_params (param_type :: acc) rest
        | Error _ -> None)
    | (_name, None) :: _ -> None
  in
  match (convert_params [] func_def.signature_params, func_def.return_type) with
  | Some param_types, Some return_type_expr -> (
      match convert return_type_expr with
      | Ok return_type -> Some (param_types, return_type)
      | Error _ -> None)
  | _ -> None

let refine_declared_params_from_expected_return
    (declared_param_types : Types.mono_type list)
    (declared_return_type : Types.mono_type)
    (actual_param_types : Types.mono_type list)
    (expected_return_type : Types.mono_type) : Types.mono_type list option =
  if List.length declared_param_types <> List.length actual_param_types then
    None
  else
    match Unify.unify declared_return_type expected_return_type with
    | Error _ -> None
    | Ok return_subst ->
        let rec unify_params subst params actuals =
          match (params, actuals) with
          | [], [] -> subst
          | declared :: rest_declared, actual :: rest_actual -> (
              let declared' = Types.apply_substitution subst declared in
              let actual' = Types.apply_substitution subst actual in
              match Unify.unify declared' actual' with
              | Ok param_subst ->
                  unify_params (Types.compose_substitution param_subst subst) rest_declared rest_actual
              | Error _ -> unify_params subst rest_declared rest_actual)
          | _ -> subst
        in
        let subst = unify_params return_subst declared_param_types actual_param_types in
        Some (List.map (Types.apply_substitution subst) declared_param_types)
...
let select_function_value_signature ?expected_type (inferred_func_type : Types.mono_type) :
    Types.mono_type list * Types.mono_type =
  let inferred_params, inferred_ret = extract_all_param_types inferred_func_type in
  match expected_type with
  | Some expected_func_type -> (
      let expected_param_types, expected_ret = extract_all_param_types expected_func_type in
      if expected_param_types <> [] && not (List.exists has_type_vars expected_param_types) then
        let selected_ret =
          if has_type_vars expected_ret then
            ...
            expected_ret
        in
        (expected_param_types, selected_ret)
      else
...
let infer_callable_body_with_signature
    ...
    ~(param_types : Types.mono_type list)
    ~(return_type : Types.mono_type)
    (body_stmt : AST.statement) : (Infer.type_map * (int, Types.mono_type list) Hashtbl.t) option =
  ...
  match
    Infer.with_inference_state infer_state (fun () ->
        Infer.type_callable inferred_map env ~type_bindings:[] ~known_param_types ~params ~return_annot:None
          ~known_return:(Some return_type) ~effect_annot:`Unspecified ~strict_return_check:false ~body:body_stmt)
  with
      ...
      (fun (m : AST.method_impl) ->
        let registered_method_opt =
          ...
          | Some registered_impl ->
              List.find_opt
                (fun (method_sig : Typecheck.Trait_registry.method_sig) ->
                  method_sig.method_name = m.impl_method_name)
...
let rec codegen_compatible_type (actual_type : Types.mono_type) (expected_type : Types.mono_type) :
    (Types.substitution, Diagnostic.t) result =
  let actual_type =
  ...
  | Types.TRecord (actual_fields, _), Types.TRecord (expected_fields, _) ->
      let rec unify_required_fields subst = function
        ...
        | (expected_field : Types.record_field_type) :: rest -> (
            match
              List.find_opt (fun (f : Types.record_field_type) -> f.name = expected_field.name) actual_fields
            with
            | None ->
                Error
                  (Diagnostic.error_no_span ~code:"type-mismatch"
                     ~message:
                       (Printf.sprintf "Cannot unify %s with %s" (Types.to_string actual_type)
...
let specialize_signature
  ...
  let rec is_callable_like (t : Types.mono_type) =
    match Types.canonicalize_mono_type t with
    | Types.TFun _ -> true
    | Types.TUnion members -> List.exists is_callable_like members
    | Types.TIntersection members -> List.exists is_callable_like members
    | Types.TArray elem -> is_callable_like elem
    | Types.THash (key, value) -> is_callable_like key || is_callable_like value
    | Types.TRecord (fields, row) -> (
        List.exists (fun (field : Types.record_field_type) -> is_callable_like field.typ) fields
        ||
        match row with
  ...
  let rec unify_params subst actual_types expected_types =
    ...
    | _ -> None
  in
  let prioritized_pairs =
    List.combine declared_param_types actual_param_types
    |> List.sort (fun (_decl_a, actual_a) (_decl_b, actual_b) ->
           compare (is_callable_like actual_a) (is_callable_like actual_b))
  in
  let declared_param_types', actual_param_types' = List.split prioritized_pairs in
...
let add_instantiation (state : mono_state) (inst : instantiation) : unit =
  if state.concrete_only && List.exists has_type_vars (inst.return_type :: inst.concrete_types) then
    ()
  else
    let concrete_types =
      List.map (normalize_instantiation_type ~concrete_only:state.concrete_only) inst.concrete_types
    in
    let return_type = normalize_instantiation_type ~concrete_only:state.concrete_only inst.return_type in
    let inst = { inst with concrete_types; return_type; type_fingerprint = fingerprint_types concrete_types } in
...
let register_user_function_value_instantiation
    (state : mono_state)
    ~(target_name : string)
    ~(param_types : Types.mono_type list)
    ~(return_type : Types.mono_type) : unit =
  ...
  if arity > 0 && not has_unresolved then
    match lookup_func_def_for_call state target_name arity with
    | None -> ()
    | Some func_def ->
        add_instantiation state
          {
...
let rec user_func_target_name_for_expr (state : mono_state) (expr : AST.expression) : string option =
  match placeholder_rewritten_expr state.placeholder_rewrite_map expr with
  | { expr = AST.TypeApply (callee, _); _ } -> user_func_target_name_for_expr state callee
  | { expr = AST.Identifier name; _ } when is_user_func state name -> Some name
  | { expr = AST.Identifier alias_name; _ } -> (
      ...
      | _ -> None)
  | { expr = AST.Function { origin = AST.PlaceholderSection; _ }; _ } as rewritten_expr ->
      hoisted_section_target_name state rewritten_expr
  | _ -> None
...
let target_name_for_value_alias (state : mono_state) (value_expr : AST.expression) : string option =
  ...
  | AST.Identifier alias_name -> Hashtbl.find_opt state.value_func_aliases alias_name
  | AST.FieldAccess ({ expr = AST.Identifier record_name; _ }, field_name) ->
      Hashtbl.find_opt state.record_field_func_aliases (field_alias_key record_name field_name)
  | _ -> None

let value_alias_type (state : mono_state) (env : Infer.type_env) (value_expr : AST.expression) :
    Types.mono_type option =
  match target_name_for_value_alias state value_expr with
  | Some target_name -> (
...
let explicit_binding_annotation_type ?(type_bindings = []) (type_annotation : AST.type_expr option) :
    Types.mono_type option =
  Option.bind type_annotation (fun type_ann ->
      match Annotation.type_expr_to_mono_type_with type_bindings type_ann with
      | Ok typ -> Some typ
      | Error _ -> None)
  ...
  | Types.TUnion members ->
      List.exists
        (function
          ...
          | _ -> false)
        members
      || List.exists contains_callable_union members
  | Types.TFun (arg, ret, _) -> contains_callable_union arg || contains_callable_union ret
  | Types.TArray elem -> contains_callable_union elem
  | Types.THash (key, value) -> contains_callable_union key || contains_callable_union value
  | Types.TRecord (fields, row) -> (
      List.exists (fun (field : Types.record_field_type) -> contains_callable_union field.typ) fields
      ||
      match row with
      | Some row_type -> contains_callable_union row_type
      | None -> false)
...
let prefer_actual_codegen_param_type ~(actual : Types.mono_type) ~(specialized : Types.mono_type) :
    Types.mono_type =
  if actual <> specialized && contains_callable_union specialized then
    actual
  else
    specialized

let rec type_imprecision_score (t : Types.mono_type) : int =
  match Types.canonicalize_mono_type t with
  | Types.TVar _ | Types.TRowVar _ -> 16
  | Types.TFun (arg, ret, _) -> type_imprecision_score arg + type_imprecision_score ret
  | Types.TArray elem -> type_imprecision_score elem
  | Types.THash (key, value) -> type_imprecision_score key + type_imprecision_score value
  | Types.TRecord (fields, row) ->
      List.fold_left
        (fun acc (field : Types.record_field_type) -> acc + type_imprecision_score field.typ)
        (match row with
        | Some row_type -> 8 + type_imprecision_score row_type
        | None -> 0)
        fields
  | Types.TUnion members ->
      let union_penalty =
        ...
        then
          ...
          4
      in
      union_penalty + List.fold_left (fun acc member -> acc + type_imprecision_score member) 0 members
  | Types.TIntersection members ->
...
let prefer_more_precise_param_type (left : Types.mono_type) (right : Types.mono_type) : Types.mono_type =
  if type_imprecision_score left <= type_imprecision_score right then
    left
  else
    right
...
let rec expr_type_from_env_or_map (env : Infer.type_env) (type_map : Infer.type_map) (expr : AST.expression) :
  ...
  match expr.expr with
  | AST.Identifier name -> (
      let map_type_opt = Hashtbl.find_opt type_map expr.id in
      let env_type_opt =
...
let arg_type_for_specialization
  ...
  | AST.Identifier alias_name -> (
      ...
      | Some target_name -> (
          match Infer.TypeEnv.find_opt target_name env with
          | Some (Types.Forall (_, func_type)) -> func_type
          | None -> expr_type_from_env_or_map env type_map expr)
      | None -> expr_type_from_env_or_map env type_map expr)
  | _ -> expr_type_from_env_or_map env type_map expr

type field_access_callable_resolution =
  | NotQualifiedCallable
  | EnumConstructorValue of {
      enum_name : string;
      variant_name : string;
      result_type : Types.mono_type;
      constructor_arity : int;
    }
  | QualifiedTraitValue of {
      trait_name : string;
      method_name : string;
      receiver_type : Types.mono_type;
      method_type_args : Types.mono_type list;
    }
  | QualifiedInherentValue of {
      method_name : string;
      receiver_type : Types.mono_type;
      method_type_args : Types.mono_type list;
    }

let refined_expr_type_for_expected
    (type_map : Infer.type_map) (expr : AST.expression) (expected_type : Types.mono_type option) :
    Types.mono_type * Types.substitution =
  let base_type = get_type type_map expr in
  match expected_type with
  | Some expected -> (
      match Unify.unify base_type expected with
      | Ok subst -> (Types.apply_substitution subst base_type, subst)
      | Error _ -> (base_type, Types.empty_substitution))
  | None -> (base_type, Types.empty_substitution)

let resolved_method_type_args_for_expr
    (method_type_args_map : (int, Types.mono_type list) Hashtbl.t)
    (subst : Types.substitution)
    (expr : AST.expression) : Types.mono_type list =
...
let method_generic_substitution
    (method_sig : Typecheck.Trait_registry.method_sig) (method_type_args : Types.mono_type list) :
    Types.substitution =
...
let trait_impl_type_from_resolved_method_type
    (trait_name : string) (method_name : string) (resolved_method_type : Types.mono_type) : Types.mono_type option
    =
  match Typecheck.Trait_registry.lookup_trait_method_with_supertraits trait_name method_name with
  | None -> None
  | Some (source_trait_def, method_sig) -> (
      match Typecheck.Trait_registry.trait_type_params source_trait_def with
      | [] -> None
      | primary_trait_param :: _ -> (
          ...
          | Ok subst ->
              Some
                (Types.apply_substitution subst (Types.TVar primary_trait_param) |> Types.canonicalize_mono_type))
      )

let qualified_trait_impl_type_from_direct_call
    (env : Infer.type_env)
    (type_map : Infer.type_map)
    ~(expected_type : Types.mono_type option)
    (expr : AST.expression)
    ...
    (trait_name : string)
    (method_name : string) : Types.mono_type option =
...
let resolve_field_access_callable
  ...
  | Some (Typecheck.Resolution_artifacts.QualifiedTraitMethod trait_name) -> (
      ...
      let resolved_method_type_args = resolved_method_type_args_for_expr method_type_args_map subst expr in
      let resolved_trait_impl_type =
        trait_impl_type_from_resolved_method_type trait_name field_name resolved_expr_type
      in
          ...
          QualifiedTraitValue
            {
              ...
              receiver_type =
                Types.canonicalize_mono_type (Option.value resolved_trait_impl_type ~default:receiver_type);
  ...
  | Some Typecheck.Resolution_artifacts.QualifiedInherentMethod -> (
      ...
      match param_types with
      | receiver_type :: _ ->
          QualifiedInherentValue
            {
              method_name = field_name;
              receiver_type = Types.canonicalize_mono_type receiver_type;
              method_type_args = resolved_method_type_args;
            }
      | [] -> NotQualifiedCallable)
  | _ -> (
      match receiver.expr with
      | AST.Identifier enum_source_name
        when Infer.classify_dotted_receiver env enum_source_name field_name = `EnumVariant -> (
          let enum_name = canonical_enum_name_for_codegen enum_source_name in
          let variant_name = canonical_variant_name_for_codegen enum_name field_name in
          let resolved_expr_type, _subst = refined_expr_type_for_expected type_map expr expected_type in
          let resolved_expr_type = Types.canonicalize_mono_type resolved_expr_type in
          let _param_types, return_type = extract_all_param_types resolved_expr_type in
          match Typecheck.Enum_registry.lookup_variant enum_name variant_name with
          | Some variant ->
              EnumConstructorValue
                {
                  enum_name;
                  variant_name;
                  result_type = Types.canonicalize_mono_type return_type;
                  constructor_arity = List.length variant.fields;
                }
          | None -> NotQualifiedCallable)
      | _ -> NotQualifiedCallable)

let rec is_callable_like_type (t : Types.mono_type) =
  match Types.canonicalize_mono_type t with
  | Types.TFun _ -> true
  | Types.TUnion members -> List.exists is_callable_like_type members
  | Types.TIntersection members -> List.exists is_callable_like_type members
  | Types.TArray elem -> is_callable_like_type elem
  | Types.THash (key, value) -> is_callable_like_type key || is_callable_like_type value
  | Types.TRecord (fields, row) -> (
      List.exists (fun (field : Types.record_field_type) -> is_callable_like_type field.typ) fields
      ||
      match row with
...
let partially_specialize_declared_params_from_noncallable_args
    (declared_param_types : Types.mono_type list) (actual_param_types : Types.mono_type list) :
    Types.mono_type list =
  let rec unify_pairs subst pairs =
    match pairs with
    | [] -> Some subst
    | (declared_param_type, actual_param_type) :: rest -> (
        let declared_param_type' = Types.apply_substitution subst declared_param_type in
        let actual_param_type' = Types.apply_substitution subst actual_param_type in
        match Unify.unify declared_param_type' actual_param_type' with
        | Ok new_subst -> unify_pairs (Types.compose_substitution new_subst subst) rest
        | Error _ -> None)
  in
  let noncallable_pairs =
    List.combine declared_param_types actual_param_types
    |> List.filter (fun (_declared_param_type, actual_param_type) ->
           not (is_callable_like_type actual_param_type))
  in
  match unify_pairs Types.empty_substitution noncallable_pairs with
  | Some subst -> List.map (Types.apply_substitution subst) declared_param_types
  | None -> declared_param_types

let refine_callable_param_types_from_user_args
    (state : mono_state)
    (env : Infer.type_env)
    (type_map : Infer.type_map)
    ~(args : AST.expression list)
    (param_types : Types.mono_type list) : Types.mono_type list =
  let rebuild_callable_type
      (target_func_type : Types.mono_type) (params : Types.mono_type list) (return_type : Types.mono_type) :
      Types.mono_type =
    let mk_fun =
  ...
  in
  let lookup_target_func_type (target_name : string) (arity : int) : Types.mono_type option =
    let env_type_opt =
      match Infer.TypeEnv.find_opt target_name env with
      | Some (Types.Forall (_, target_func_type))
        when Option.is_some (extract_param_types_exact arity target_func_type) ->
          Some target_func_type
      | _ -> None
    in
    match env_type_opt with
    | Some target_func_type -> Some target_func_type
    | None -> (
        match
          List.find_opt
            (fun (fd : func_def) -> fd.name = target_name && List.length fd.params = arity)
            state.func_defs
        with
        | Some func_def -> Hashtbl.find_opt type_map func_def.func_expr_id
        | None -> None)
  in
  List.map2
    (fun (arg_expr : AST.expression) (param_type : Types.mono_type) ->
      ...
      | Some target_name -> (
          ...
          match callable_signature_exact arity param_type with
          | Some (hint_params, _hint_ret) when hint_params <> [] && not (List.exists has_type_vars hint_params)
            -> (
              match lookup_target_func_type target_name arity with
              | Some target_func_type -> (
                  match extract_param_types_exact arity target_func_type with
                  | Some (target_params, target_ret) -> (
                      match specialize_signature target_params target_ret hint_params with
                      | Some (specialized_params, specialized_ret)
                        when not (List.exists has_type_vars (specialized_ret :: specialized_params)) ->
                          rebuild_callable_type target_func_type specialized_params specialized_ret
                      | _ -> param_type)
                  | None -> param_type)
              | None -> param_type)
          | _ -> param_type)
...
let register_user_func_call_instantiation
  ...
  if List.exists has_imprecise_callable_arg_type raw_arg_types then
    ...
    let declared_signature_opt =
      match Option.bind func_def_opt func_def_declared_signature with
    ...
    let specialized_declared_signature =
      ...
      | _ -> None
    in
    let inferred_call_return =
      match expected_type with
      | Some expected when not (has_unresolved_codegen_type expected) -> expected
      | _ -> get_type type_map call_expr
    in
...
let specialized_user_call_param_types
  ...
  | Some (Types.Forall (_, func_type)) -> (
      ...
      | Some (declared_param_types, declared_return_type) ->
          ...
          in
          let partially_specialized_params =
            partially_specialize_declared_params_from_noncallable_args declared_param_types arg_types
          in
          let selected_param_types =
            let base_param_types =
              match specialize_signature declared_param_types declared_return_type arg_types with
...
let type_from_env_or_map_for_collect (env : Infer.type_env) (type_map : Infer.type_map) (expr : AST.expression) :
    Types.mono_type option =
  match expr.expr with
  | AST.Identifier id -> (
...
let rec collect_forward_call_arg_types_expr
  ...
  let nested =
    ...
    | AST.TypeCheck (e, _) -> collect_forward_call_arg_types_expr name type_map env e
    | AST.If (cond, cons, alt) -> (
        collect_forward_call_arg_types_expr name type_map env cond
        @ collect_forward_call_arg_types_stmt name type_map env cons
        @
        match alt with
        | Some s -> collect_forward_call_arg_types_stmt name type_map env s
        | None -> [])
    | AST.Call (callee, args) ->
        collect_forward_call_arg_types_expr name type_map env callee
        @ List.concat_map (collect_forward_call_arg_types_expr name type_map env) args
    | AST.Array elements -> List.concat_map (collect_forward_call_arg_types_expr name type_map env) elements
    | AST.Hash pairs ->
        List.concat_map
          (fun (k, v) ->
            collect_forward_call_arg_types_expr name type_map env k
            @ collect_forward_call_arg_types_expr name type_map env v)
          pairs
    | AST.Index (container, index) ->
        collect_forward_call_arg_types_expr name type_map env container
        @ collect_forward_call_arg_types_expr name type_map env index
    | AST.Function _ -> []
    | AST.EnumConstructor (_, _, args) ->
        List.concat_map (collect_forward_call_arg_types_expr name type_map env) args
    | AST.Match (scrutinee, arms) ->
        collect_forward_call_arg_types_expr name type_map env scrutinee
        @ List.concat_map
            (fun (arm : AST.match_arm) -> collect_forward_call_arg_types_expr name type_map env arm.body)
            arms
    | AST.Try { tried; fallback; _ } ->
        collect_forward_call_arg_types_expr name type_map env tried
        @ (fallback
    ...
    | AST.RecordLit (fields, spread) -> (
        List.concat_map
          (fun (field : AST.record_field) ->
            match field.field_value with
            | Some v -> collect_forward_call_arg_types_expr name type_map env v
            | None -> [])
          fields
        @
        match spread with
        | Some s -> collect_forward_call_arg_types_expr name type_map env s
        | None -> [])
    | AST.FieldAccess (receiver, _) -> collect_forward_call_arg_types_expr name type_map env receiver
    | AST.MethodCall { mc_receiver; mc_args; _ } ->
        collect_forward_call_arg_types_expr name type_map env mc_receiver
        @ List.concat_map (collect_forward_call_arg_types_expr name type_map env) mc_args
    | AST.BlockExpr stmts -> List.concat_map (collect_forward_call_arg_types_stmt name type_map env) stmts
  in
  match expr.expr with
...
and collect_forward_call_arg_types_stmt
    (name : string) (type_map : Infer.type_map) (env : Infer.type_env) (stmt : AST.statement) :
    Types.mono_type list list =
  match stmt.stmt with
  | AST.Let { value; _ } -> collect_forward_call_arg_types_expr name type_map env value
  | AST.Return e | AST.ExpressionStmt e -> collect_forward_call_arg_types_expr name type_map env e
  | AST.Block stmts -> List.concat_map (collect_forward_call_arg_types_stmt name type_map env) stmts
  | AST.ImplDef impl ->
      List.concat_map
...
let resolve_local_function_type_from_calls_for_collect
  ...
  if declared_arity = 0 then
    ...
    let call_arg_types =
      List.concat_map (collect_forward_call_arg_types_stmt name type_map env) following_stmts
...
let rec collect_insts_stmt
    ...
    (type_map : Infer.type_map)
    (env : Infer.type_env)
    (stmt : AST.statement) : Infer.type_env =
  match stmt.stmt with
  | AST.Let let_binding ->
      let value_expr = placeholder_rewritten_expr state.placeholder_rewrite_map let_binding.value in
      ...
      in
      let expr_type =
        if has_type_vars expr_type && fst (extract_all_param_types expr_type) <> [] then
          match
            resolve_local_function_type_from_calls_for_collect let_binding.name expr_type type_map env
              following_stmts
          with
      ...
      (match value_expr.expr with
      | AST.Function _ -> (
          match hoisted_local_func_target_name state let_binding.name value_expr.id with
      ...
      | AST.FieldAccess ({ expr = AST.Identifier record_name; _ }, field_name) -> (
          ...
          | None -> Hashtbl.remove state.value_func_aliases let_binding.name)
      | AST.RecordLit (fields, spread) ->
          let explicit_field_names = List.map (fun (field : AST.record_field) -> field.field_name) fields in
          List.iter
            (fun (field : AST.record_field) ->
              match field.field_value with
              | Some { expr = AST.Identifier target_name; _ } when is_user_func state target_name ->
                  Hashtbl.replace state.record_field_func_aliases
                    ...
                    target_name
              | Some { expr = AST.Identifier alias_name; _ } -> (
                  match Hashtbl.find_opt state.value_func_aliases alias_name with
            ...
            fields;
          (match spread with
          | Some { expr = AST.Identifier base_name; _ } ->
              Hashtbl.iter
                (fun key target_name ->
                  let prefix = base_name ^ "." in
                  if String.length key > String.length prefix && String.sub key 0 (String.length prefix) = prefix
                  then
                    let field_name =
                      String.sub key (String.length prefix) (String.length key - String.length prefix)
                    in
                    if not (List.mem field_name explicit_field_names) then
                      Hashtbl.replace state.record_field_func_aliases
                        (field_alias_key let_binding.name field_name)
                        target_name)
                state.record_field_func_aliases
          | _ -> ());
  ...
  | AST.ImplDef impl ->
      ...
      if impl.impl_type_params = [] then (
        ...
        | Some _trait_def ->
            List.iter
              (fun ((source_trait_def, m) :
                     Typecheck.Trait_registry.trait_def * Typecheck.Trait_registry.method_sig) ->
                match m.method_default_impl with
  ...
  | AST.InherentImplDef impl ->
      ...
      if not (has_type_vars for_type) then
        List.iter
          (fun (m : AST.method_impl) ->
...
and collect_insts_stmt_list
    (state : mono_state) (type_map : Infer.type_map) (env : Infer.type_env) (stmts : AST.statement list) :
    Infer.type_env =
  let rec go env_acc = function
    | [] -> env_acc
    | stmt :: rest ->
        let env' = collect_insts_stmt ~following_stmts:rest state type_map env_acc stmt in
        go env' rest
  in
  go env stmts
        ...
        let internal_var_subst =
          if template_method.method_generic_internal_vars = [] || method_type_args = [] then
            ...
            List.fold_left
              (fun acc (generic_name, internal_name) ->
                match
                  List.assoc_opt generic_name (List.combine template_method.method_generic_names method_type_args)
                with
                | Some concrete -> Types.SubstMap.add internal_name concrete acc
                | None -> acc)
...
and register_trait_object_support_for_type
  ...
  |> List.iter (fun (method_info : dyn_callable_method) ->
         register_impl_method_use state type_map env
           ~trait_name:(dyn_dispatch_trait_name source_type method_info)
...
and collect_insts_expr
  ...
  | AST.Identifier name -> (
      let target_name_opt =
        if is_user_func state name then
          ...
          match Hashtbl.find_opt state.value_func_aliases name with
          | Some target_name when is_user_func state target_name -> Some target_name
          | _ -> None
      in
      match target_name_opt with
      ...
      | Some target_name ->
          let inferred_func_type =
            ...
            | None -> (
                ...
                | None -> get_type type_map expr)
          in
          let param_types, return_type = select_function_value_signature ?expected_type inferred_func_type in
          register_user_function_value_instantiation state ~target_name ~param_types ~return_type)
  | AST.Function fn_expr ->
      (match (fn_expr.origin, hoisted_section_target_name state expr) with
      | AST.PlaceholderSection, Some target_name ->
          let inferred_func_type = get_type type_map expr in
          let param_types, return_type = select_function_value_signature ?expected_type inferred_func_type in
          register_user_function_value_instantiation state ~target_name ~param_types ~return_type
      | _ -> ());
      let param_names = List.map fst fn_expr.params in
      ...
      ignore (collect_insts_stmt state type_map body_env fn_expr.body)
  | AST.Prefix (op, e) -> (
      collect_insts_expr state type_map env e;
      match op with
      | "-" -> (
  ...
  | AST.Call (func, args) -> (
      ...
      let rec collect_args_with_expected remaining_args remaining_expected =
        ...
        | arg :: rest_args, expected_type :: rest_expected ->
            (match resolved_user_func_call_target state arg with
            | Some target_name -> (
                let matching_defs =
                  List.filter (fun (fd : func_def) -> fd.name = target_name) state.func_defs
                  |> List.filter_map (fun (fd : func_def) ->
                         match callable_signature_exact (List.length fd.params) expected_type with
                         | Some (params, ret) when not (List.exists has_type_vars (ret :: params)) ->
                             Some (fd, params, ret)
                         | _ -> None)
                in
                match matching_defs with
                | [ (fd, params, ret) ] ->
                    add_instantiation state
                      {
                        func_name = fd.name;
                        module_path = state.module_path;
                        func_expr_id = fd.func_expr_id;
                        func_arity = List.length fd.params;
                        concrete_only_mode = state.concrete_only;
                        concrete_types = params;
                        type_fingerprint = fingerprint_types params;
                        return_type = ret;
                      }
                | _ -> ())
            | None -> ());
      ...
      match user_call_target with
      | Some target_name ->
          register_user_func_call_instantiation state type_map env ~expected_type ~target_name ~args
            ~call_expr:expr
      | None -> ())
  ...
  | AST.RecordLit (fields, spread) -> (
      ...
      match spread with
      | Some expr ->
          collect_insts_expr state type_map env expr;
          let spread_fields =
            match Types.canonicalize_mono_type (get_type type_map expr) with
            | Types.TRecord (fields, row) ->
                let normalized_fields, _ = normalize_record_row_type fields row in
                normalized_fields
            | _ -> []
          in
          List.iter
            (fun (result_field : Types.record_field_type) ->
  ...
  | AST.FieldAccess (receiver, field_name) -> (
      match
        resolve_field_access_callable ~call_resolution_map:state.call_resolution_map
          ~method_type_args_map:state.method_type_args_map type_map env ~expected_type expr receiver field_name
      with
      ...
      | QualifiedTraitValue { trait_name; method_name; receiver_type; method_type_args } ->
          ...
          then
            ...
            register_impl_method_use state type_map env ~trait_name ~method_name ~for_type:receiver_type
              ~method_type_args
      | QualifiedInherentValue { method_name; receiver_type; method_type_args } ->
  ...
  | AST.MethodCall { mc_receiver = receiver; mc_method = method_name; mc_args = args; _ } -> (
      ...
      | _ -> (
          ...
          | Some (Typecheck.Resolution_artifacts.QualifiedTraitMethod trait_name) -> (
              let for_type =
                match
                  qualified_trait_impl_type_from_direct_call env type_map ~expected_type expr args trait_name
                    method_name
                with
                | Some typ -> typ
                | None -> Types.canonicalize_mono_type (method_dispatch_type env type_map (List.hd args))
              in
              match for_type with
              | Types.TTraitObject _ -> ()
              | _ ->
          ...
          | Some Typecheck.Resolution_artifacts.FieldFunctionCall -> (
              ...
              | AST.Identifier record_name -> (
                  ...
                  with
                  | Some target_name when is_user_func state target_name ->
                      register_user_func_call_instantiation state type_map env ~expected_type:None ~target_name
                        ~args ~call_expr:expr
...
let with_method_type_args_map
    (state : emit_state) (method_type_args_map : (int, Types.mono_type list) Hashtbl.t) (f : unit -> 'a) : 'a =
...
let with_type_bindings (state : emit_state) (type_bindings : (string * Types.mono_type) list) (f : unit -> 'a) :
    'a =
  let previous = state.current_type_bindings in
  state.current_type_bindings <- type_bindings @ previous;
  Fun.protect ~finally:(fun () -> state.current_type_bindings <- previous) f
...
let extern_func_exn (state : mono_state) (shim_key : string) : Typecheck.Resolution_artifacts.extern_func =
  ...
  | Some func -> func
  | None ->
      failwith (Printf.sprintf "Codegen error: missing shim extern declaration artifact for key %S" shim_key)
...
let source_error_context_expr
    (state : emit_state) (expr : AST.expression) (enum_name : string) (variant_name : string) : string option =
  match
    (Typecheck.Enum_registry.lookup enum_name, Typecheck.Enum_registry.lookup_variant enum_name variant_name)
  with
  | Some enum_def, Some variant when enum_def_is_error enum_def ->
      ...
      Some
        (error_context_expr state.mono
           ~message:(error_variant_message enum_name variant)
           ~file_id ~function_name ~position:expr.pos)
  | _ -> None
...
let maybe_attach_source_error_context
  ...
  | Some context_expr ->
      ...
      Printf.sprintf "(func(__err %s) %s { __err.Context = %s; return __err })(%s)" go_type go_type context_expr
        emitted
...
let enum_constructor_expected_arg_types (enum_type : Types.mono_type) (enum_name : string) (variant_name : string)
    : Types.mono_type list =
  let enum_name = canonical_enum_name_for_codegen enum_name in
  let variant_name = canonical_variant_name_for_codegen enum_name variant_name in
  ...
  | Types.TRecord (expected_fields, expected_row) -> (
      ...
      | Types.TRecord (actual_fields, actual_row) ->
          ...
          let projected_fields =
            ...
            |> List.map (fun (field : Types.record_field_type) ->
                   ...
                   let value_expr =
                     match
                       List.find_opt
                         (fun (candidate : Types.record_field_type) -> candidate.name = field.name)
                         actual_fields
                     with
                     | Some actual_field ->
          ...
          let needs_projection =
            ...
            || List.exists
                 (fun ((field : Types.record_field_type), value_expr) ->
                   value_expr <> direct_field_value field.name)
                 projected_fields
          in
      ...
      | Types.TRecord (expected_fields, expected_row) -> (
          ...
          let actual_type_opt =
            let raw_actual_type_opt =
              ...
              | _ -> type_from_env_or_map expr
            in
            Option.map (normalize_type_for_codegen ~concrete_only:state.mono.concrete_only) raw_actual_type_opt
          in
          match actual_type_opt with
          | Some (Types.TRecord (actual_fields, actual_row)) ->
              project_value_to_expected_type state type_map env
                (Types.TRecord (actual_fields, actual_row))
                (Types.TRecord (expected_fields, None))
                expr_str
          | _ -> expr_str)
      | _ -> expr_str)
...
let rec copy_specialized_expr_types
  ...
  | AST.TypeApply (callee, _) -> copy_specialized_expr_types source_map target_map specialization_subst callee
...
let copy_specialized_synthetic_types
    (source_map : Infer.type_map) (target_map : Infer.type_map) (specialization_subst : Types.substitution) : unit
    =
  Hashtbl.iter
    (fun expr_id ty ->
      if expr_id < 0 then
...
let substitute_path_in_expr (path : Type_narrowing.path) (replacement_name : string) (expr : AST.expression) :
    AST.expression =
  Type_narrowing.substitute_path_in_expr path
    ~replace:(Type_narrowing.replacement_identifier replacement_name)
    expr

let substitute_path_in_stmt (path : Type_narrowing.path) (replacement_name : string) (stmt : AST.statement) :
    AST.statement =
...
let rec emit_expr
  ...
  let emitted =
    ...
    | AST.Call (func, args) -> emit_call ?expected_type state type_map env expr func args
    | AST.Array elements ->
    ...
    | AST.Function f -> (
        ...
        let selected_func_type =
          if should_use_expected then
            ...
            inferred_func_type
        in
        match (f.origin, hoisted_section_target_name state.mono expr) with
        | AST.PlaceholderSection, Some target_name ->
            let param_types, return_type = select_function_value_signature ?expected_type selected_func_type in
            let has_unresolved = List.exists has_type_vars (return_type :: param_types) in
            if List.length param_types > 0 && not has_unresolved then (
              ...
              in
              match
                ( lookup_func_def_for_call state.mono target_name arity,
                  instantiated_func_for_signature state.mono target_name param_types return_type )
              with
    ...
    | AST.Try { tried; fallback = Some fallback; _ } ->
        emit_try_or_expr state type_map env ~try_expr:expr ~tried ~fallback
    ...
    | AST.FieldAccess (receiver, variant_name) -> (
        match
          resolve_field_access_callable ~call_resolution_map:state.mono.call_resolution_map
            ~method_type_args_map:state.mono.method_type_args_map type_map env ~expected_type expr receiver
            variant_name
        with
        | EnumConstructorValue { enum_name; variant_name; result_type; constructor_arity } ->
            let enum_type =
        ...
        | QualifiedTraitValue { trait_name; method_name; receiver_type; method_type_args } ->
            let receiver_type =
              normalize_type_for_codegen ~concrete_only:state.mono.concrete_only receiver_type
            in
            let type_suffix =
              if method_type_args = [] then
                mangle_type receiver_type
              else
                fingerprint_types (receiver_type :: method_type_args)
            in
            trait_method_func_name trait_name method_name type_suffix
        | QualifiedInherentValue { method_name; receiver_type; method_type_args } ->
            register_inherent_call_use ~allow_erased_unresolved:true state.mono ~method_name ~receiver_type
              ~method_type_args;
            let receiver_type =
              normalize_type_for_codegen ~concrete_only:state.mono.concrete_only receiver_type
            in
            let type_suffix =
              if method_type_args = [] then
                ...
                fingerprint_types (receiver_type :: method_type_args)
            in
            inherent_method_func_name method_name type_suffix
        | NotQualifiedCallable ->
    ...
    | AST.MethodCall { mc_receiver = receiver; mc_method = variant_name; mc_args = args; _ } -> (
        ...
        | _ -> (
            ...
            match call_resolution with
            | Some (Typecheck.Resolution_artifacts.QualifiedTraitMethod trait_name) -> (
                let raw_trait_impl_type =
                  match
                    qualified_trait_impl_type_from_direct_call env type_map ~expected_type expr args trait_name
                      variant_name
                  with
                  | Some typ -> typ
                  | None -> method_dispatch_type env type_map (List.hd args)
                in
                let first_arg_type =
                  normalize_type_for_codegen ~concrete_only:state.mono.concrete_only raw_trait_impl_type
                in
                    ...
                    let arg_strs =
                      List.map
                        (fun arg ->
                          let arg_type =
                            normalize_type_for_codegen ~concrete_only:state.mono.concrete_only
                              (method_dispatch_type env type_map arg)
                          in
                          emit_expr_for_expected_type state type_map env arg_type arg)
                        args
                    in
              ...
              with_indent_delta state 2 (fun () ->
                  ...
                  let last_str =
                    ...
                    | AST.ExpressionStmt e ->
                        ...
                        inner_ind
                        ^ "return "
                        ^ emit_expr_for_expected_type state type_map env' result_type e
                        ^ "\n"
                    | AST.Return e ->
                        let inner_ind = indent_str state in
                        inner_ind
...
and emit_dynamic_trait_call
  ...
  | Types.TTraitObject traits ->
      ...
      Printf.sprintf "(func() %s { __dyn := %s; __witness := __dyn.witness.(%s); return __witness.%s(%s) })()"
...
and coercion_source_type_for_codegen
  ...
  in
  let actual_type = normalize_type_for_codegen ~concrete_only:state.mono.concrete_only (get_type type_map expr) in
  match
    Infer.classify_trait_object_compatibility
      ~mk_error:(fun ~code ~message -> Diagnostic.error_no_span ~code ~message)
...
and emit_infix state type_map env left op right =
  ...
  | "<" ->
      if is_ord_primitive then
        ...
        Printf.sprintf "((ord_compare_%s(%s, %s)).Tag == %s)" type_suffix left_str right_str ordering_less_tag
  | ">" ->
      if is_ord_primitive then
        Printf.sprintf "(%s > %s)" left_str right_str
      else
        Printf.sprintf "((ord_compare_%s(%s, %s)).Tag == %s)" type_suffix left_str right_str ordering_greater_tag
  | "<=" ->
      if is_ord_primitive then
        Printf.sprintf "(%s <= %s)" left_str right_str
      else
...
and emit_type_switch
    state type_map env _if_expr checked_expr path narrow_type complement_type_opt cons alt result_type =
  ...
  let checked_expr_str = emit_expr state type_map env checked_expr in
  let use_subject_temp = not (Type_narrowing.is_identifier_path path) in
  let subject_var = Type_narrowing.temp_name path "subject" in
  let subject_expr =
    if use_subject_temp then
      subject_var
    else
      checked_expr_str
  in
  let subject_binding_prefix =
    if use_subject_temp then
      ind ^ "    " ^ subject_var ^ " := " ^ checked_expr_str ^ "\n"
    else
      ""
  in

  (* Create narrowed variable name *)
  let narrowed_var = Type_narrowing.temp_name path "typed" in
  ...
  let emit_branch_with_var var_to_use narrow_env stmt =
    let stmt = substitute_path_in_stmt path var_to_use stmt in
    ...
    | AST.ExpressionStmt e -> var_marker ^ emit_return_expr narrow_env e
    | _ -> var_marker ^ fst (emit_stmt state type_map narrow_env stmt)
  in

  (* Emit branch with complement type assertion if needed *)
  let emit_branch_with_complement var_to_use complement_type_opt narrow_env stmt =
    ...
    let type_assertion_prefix =
      ...
      | Some complement_type when complement_type <> Types.TUnion [] ->
          ...
          inner_ind ^ "        " ^ var_to_use ^ " := " ^ subject_expr ^ ".(" ^ complement_type_str ^ ")\n"
      | _ -> ""
    in
    ...
    let stmt =
      match complement_type_opt with
      | Some _ when complement_type_opt <> Some (Types.TUnion []) -> substitute_path_in_stmt path var_to_use stmt
      | _ -> stmt
    in
    match stmt.AST.stmt with
    | AST.Block stmts -> (
    ...
    | AST.ExpressionStmt e -> type_assertion_prefix ^ var_marker ^ emit_return_expr narrow_env e
  ...
  let alt_str =
    ...
    | Some alt_stmt ->
        ...
        let complement_var = Type_narrowing.temp_name path "complement" in
        let complement_env =
          match complement_type_opt with
          | Some complement_type -> Infer.TypeEnv.add complement_var (Types.Forall ([], complement_type)) env
          | None -> env
        in
        emit_branch_with_complement complement_var complement_type_opt complement_env alt_stmt
    | None ->
        inner_ind ^ "        return "
        ^ (if result_type = Types.TNull then
             "struct{}{}"
...
and emit_type_switch_to_target
    ...
    | AST.Block stmts -> (
        ...
        | last :: rest ->
            ...
            let last_str =
              ...
              | AST.Return e ->
                  inner_ind ^ "    return " ^ emit_expr_for_current_return state type_map env' e ^ "\n"
              | _ -> fst (emit_stmt state type_map env' last)
            in
  ...
  let default_branch =
    ...
    | Some alt_stmt -> (
        ...
        | Some complement_type when complement_type <> Types.TUnion [] ->
            let complement_var = Type_narrowing.temp_name path "complement" in
            let complement_type_str = type_to_go state.mono complement_type in
            let alt_stmt' = substitute_path_in_stmt path complement_var alt_stmt in
            let complement_env = Infer.TypeEnv.add complement_var (Types.Forall ([], complement_type)) env in
            let assertion =
              inner_ind ^ "        " ^ complement_var ^ " := " ^ subject_expr ^ ".(" ^ complement_type_str ^ ")\n"
            in
  ...
  in
  let switch_code =
    Printf.sprintf "%s%sswitch %s := %s.(type) {\n%scase %s:\n%s%s%s}\n" subject_binding_prefix ind narrowed_var
      subject_expr ind narrow_type_str cons_str default_branch ind
  in
  let unit_tail =
    if result_type <> Types.TNull then
      ""
...
and emit_if state type_map env if_expr cond cons alt =
  ...
  | Some info -> (
      let emit_single_branch branch_env stmt =
        let emit_return_expr branch_env e =
          if result_type = Types.TNull then
            inner_ind
            ^ "    _ = "
        ...
        let branch_str =
          ...
          | AST.Block stmts -> (
              ...
              | [] ->
                  ...
                  ^ "\n"
              | last :: rest ->
                  let prefix, env' = emit_stmts state type_map branch_env (List.rev rest) in
                  let last_str =
                    match last.stmt with
                    | AST.ExpressionStmt e -> emit_return_expr env' e
                    | AST.Return e -> emit_return_expr env' e
                  ...
                  in
                  prefix ^ last_str)
          | AST.ExpressionStmt e -> emit_return_expr branch_env e
          | _ -> fst (emit_stmt state type_map branch_env stmt)
        in
        Printf.sprintf "func() %s {\n%s%s}()" result_type_str ind branch_str
      in
      match type_check_static_outcome (Some info.current_type) info.narrow_type with
      | `Always_true -> emit_single_branch env cons
      | `Always_false -> (
          match alt with
          | Some alt_stmt -> emit_single_branch env alt_stmt
          | None ->
              Printf.sprintf "func() %s {\n%s    return %s\n%s}()" result_type_str ind
                (if result_type = Types.TNull then
                   "struct{}{}"
                 else
                   "nil")
      ...
      | `Runtime_check complement_type_opt ->
          emit_type_switch state type_map env if_expr info.checked_expr info.path info.narrow_type
            complement_type_opt cons alt result_type)
  | None ->
  ...
  | Some info -> (
      let emit_branch_to_target branch_env stmt =
        ...
        | AST.ExpressionStmt e ->
            with_indent_delta state 1 (fun () -> emit_expr_to_target state type_map branch_env e value_target)
        | AST.Return e ->
            inner_ind ^ "    return " ^ emit_expr_for_current_return state type_map branch_env e ^ "\n"
        | _ -> fst (emit_stmt state type_map branch_env stmt)
      in
      match type_check_static_outcome (Some info.current_type) info.narrow_type with
      | `Always_true -> emit_branch_to_target env cons
      | `Always_false -> (
          match alt with
          | Some alt_stmt -> emit_branch_to_target env alt_stmt
          | None -> (
              if result_type <> Types.TNull then
                ""
              else
      ...
      | `Runtime_check complement_type_opt ->
          emit_type_switch_to_target state type_map env info.checked_expr info.path info.narrow_type
            complement_type_opt result_type cons alt target)
  | None ->
      let cond_str = emit_expr state type_map env cond in
      let emit_branch stmt =
        match stmt.AST.stmt with
...
and emit_call ?expected_type state type_map env call_expr func args =
  ...
  let call_param_types =
    ...
    | Some target_name -> (
        ...
        | None -> (
            ...
            | Some inst -> inst.concrete_types
            | None when not (List.exists has_type_vars actual_arg_types) -> actual_arg_types
            | None -> (
  ...
  let call_param_types =
    let call_result_type =
      ...
      | _ -> get_type type_map call_expr
    in
    let merge_refined candidate =
      match candidate with
      | Some refined_params when List.length refined_params = List.length call_param_types ->
          List.map2 prefer_more_precise_param_type call_param_types refined_params
      | _ -> call_param_types
    in
    let declared_signature_refinement =
      match user_call_target with
      | Some target_name -> (
          match lookup_func_def_for_call state.mono target_name (List.length args) with
          | Some func_def -> (
              match func_def_declared_signature func_def with
              | Some (declared_params, declared_return) ->
                  refine_declared_params_from_expected_return declared_params declared_return actual_arg_types
                    call_result_type
              | None -> None)
          | None -> None)
      | None -> None
    in
    match declared_signature_refinement with
    | Some _ -> merge_refined declared_signature_refinement
    | None -> (
        match call_signature_opt with
  ...
  | _ -> (
          ...
          | Some (resolved_name, type_args) -> (
              ...
              | `Wrapper payload_types ->
                  let args_strs =
                    ...
                    | exception Invalid_argument _ ->
                        failwith
                          (Printf.sprintf "Codegen error: named wrapper constructor %s argument artifact mismatch"
                             type_name)
                  in
      ...
      | _ when Option.is_some user_call_target ->
          ...
          let mangle_user_instantiation (inst : instantiation) =
            match lookup_func_def_for_call state.mono target_name (List.length args) with
            | Some func_def
              when is_hoisted_section_func_name inst.func_name || func_def_requires_return_mangle func_def ->
                mangle_func_name_with_return inst.func_name inst.concrete_types inst.return_type
          ...
          let param_types, mangled_name =
            if List.exists has_type_vars call_param_types then
              match instantiated_func_for_args state.mono target_name actual_arg_types with
              | Some inst -> (inst.concrete_types, mangle_user_instantiation inst)
              | None -> (
              ...
              match instantiated_func_for_signature state.mono target_name call_param_types call_return_type with
              | Some inst -> (inst.concrete_types, mangle_user_instantiation inst)
              | None -> (
                  match lookup_func_def_for_call state.mono target_name (List.length args) with
                  | Some func_def ->
                      let inst =
                        {
                          ...
                          concrete_only_mode = state.mono.concrete_only;
                          concrete_types = call_param_types;
                          type_fingerprint = fingerprint_types call_param_types;
                          return_type = call_return_type;
                        }
                      in
                      add_instantiation state.mono inst;
                      (inst.concrete_types, mangle_user_instantiation inst)
                  | None -> (call_param_types, mangle_func_name target_name call_param_types))
          in
          let () =
...
and emit_function_expr ?func_type_override state type_map env func_expr params body =
  ...
  let body_str =
    with_return_type state return_type (fun () -> emit_func_body state body_type_map body_env body)
  in
...
and result_type_parts_for_codegen (typ : Types.mono_type) :
    (string * Types.mono_type list * Types.mono_type * Types.mono_type) option =
  match Types.canonicalize_mono_type typ with
  | Types.TEnum (enum_name, [ ok_type; error_type ]) -> (
      match Typecheck.Enum_registry.lookup enum_name with
      | Some enum_def when Typecheck.Enum_registry.source_name enum_def = "Result" ->
          Some (enum_name, [ ok_type; error_type ], ok_type, error_type)
      | _ when Typecheck.Display_names.display_binding_name enum_name = "Result" ->
          Some (enum_name, [ ok_type; error_type ], ok_type, error_type)
      | _ -> None)
  | _ -> None

and option_type_parts_for_codegen (typ : Types.mono_type) :
    (string * Types.mono_type list * Types.mono_type) option =
  match Types.canonicalize_mono_type typ with
  | Types.TEnum (enum_name, [ value_type ]) -> (
      match Typecheck.Enum_registry.lookup enum_name with
      | Some enum_def when Typecheck.Enum_registry.source_name enum_def = "Option" ->
          Some (enum_name, [ value_type ], value_type)
      | _ when Typecheck.Display_names.display_binding_name enum_name = "Option" ->
          Some (enum_name, [ value_type ], value_type)
      | _ -> None)
  | _ -> None

and enum_payload_expr_for_codegen
    (state : emit_state)
    ~(enum_name : string)
  ...
  let enum_def =
    ...
    | None -> failwith (Printf.sprintf "Codegen error: unknown enum %s" enum_name)
  in
  let layout = analyze_enum_layout state.mono enum_def type_args in
  match List.assoc_opt variant_name layout.variant_maps with
  | None ->
      failwith (Printf.sprintf "Codegen error: missing enum layout for variant %s.%s" enum_name variant_name)
  | Some mappings -> (
      match List.find_opt (fun (pos, _) -> pos = position) mappings with
      | None ->
          failwith
            (Printf.sprintf "Codegen error: missing enum payload layout for variant %s.%s field %d" enum_name
               variant_name position)
      | Some (_, mapping) ->
          let field_expr = Printf.sprintf "%s.%s" value_expr mapping.data_field_name in
          if mapping.boxed then
            Printf.sprintf "(*%s)" field_expr
          else
            field_expr)

and emit_try_binding
    (state : emit_state)
    (type_map : Infer.type_map)
    (env : Infer.type_env)
    ~(binding_name : string)
  ...
  let result_enum_name, result_type_args, _success_type, _inner_error_type =
    ...
    | Some parts -> parts
    | None -> failwith "Codegen error: try expression expected Result tried type"
  in
  let outer_result_type =
    match state.current_return_type with
    | Some return_type -> Types.canonicalize_mono_type return_type
    | None -> failwith "Codegen error: try expression emitted without current return type"
  in
  let _outer_enum_name, _outer_type_args, _outer_ok_type, outer_error_type =
    match result_type_parts_for_codegen outer_result_type with
    | Some parts -> parts
    | None -> failwith "Codegen error: try expression expected Result current return type"
  in
  track_enum_inst state.mono tried_type;
  track_enum_inst state.mono outer_result_type;
  ...
  let failure_value =
    ...
    | Some (target_type, target_variant) ->
        ...
        let outer_error_type = Types.canonicalize_mono_type outer_error_type in
  ...
  in
  let outer_result_go_name = mangle_type outer_result_type in
  let binding_go_type = type_to_go state.mono binding_type in
  Printf.sprintf
    "%svar %s %s\n%s%s := %s\n%sswitch %s.Tag {\n%scase %s_Success_tag:\n%s    %s = %s\n%scase %s_Failure_tag:\n%s    return %s_Failure(%s)\n%sdefault:\n%s    panic(%S)\n%s}\n%s_ = %s\n"
    ind binding_name binding_go_type ind result_var result_expr ind result_var ind result_go_name ind binding_name
    success_payload ind result_go_name ind outer_result_go_name failure_value ind ind
    "unexpected Result tag in try expression" ind ind binding_name

and emit_option_try_binding
    (state : emit_state)
    (type_map : Infer.type_map)
    (env : Infer.type_env)
    ~(binding_name : string)
  ...
  let option_enum_name, option_type_args, _value_type =
    ...
    | Some parts -> parts
    | None -> failwith "Codegen error: try expression expected Option tried type"
  in
  let outer_option_type =
    match state.current_return_type with
    | Some return_type -> Types.canonicalize_mono_type return_type
    | None -> failwith "Codegen error: Option try expression emitted without current return type"
  in
  let _outer_enum_name, _outer_type_args, _outer_value_type =
    match option_type_parts_for_codegen outer_option_type with
    | Some parts -> parts
    | None -> failwith "Codegen error: Option try expression expected Option current return type"
  in
  track_enum_inst state.mono tried_type;
  track_enum_inst state.mono outer_option_type;
  let result_var = fresh_temp_name state "__try_option" in
  let result_expr = emit_expr_for_expected_type state type_map env tried_type tried in
  let result_go_name = mangle_type tried_type in
  let outer_option_go_name = mangle_type outer_option_type in
  let some_payload =
  ...
  Printf.sprintf
    "%svar %s %s\n%s%s := %s\n%sswitch %s.Tag {\n%scase %s_Some_tag:\n%s    %s = %s\n%scase %s_None_tag:\n%s    return %s_None()\n%sdefault:\n%s    panic(%S)\n%s}\n%s_ = %s\n"
    ind binding_name binding_go_type ind result_var result_expr ind result_var ind result_go_name ind binding_name
    some_payload ind result_go_name ind outer_option_go_name ind ind "unexpected Option tag in try expression" ind
    ind binding_name

and wrap_failure_type_for_codegen
    (state : emit_state) ~(inner_error_type : Types.mono_type) ~(target_type : string) ~(target_variant : string)
    : Types.mono_type * string * string =
  let target_enum_name = canonical_enum_name_for_codegen target_type in
  let target_variant = canonical_variant_name_for_codegen target_enum_name target_variant in
  match
  ...
  | Some enum_def, Some variant ->
      ...
      let rec first_unifying_field = function
        | [] ->
            failwith
              (Printf.sprintf "Codegen error: wrap variant %s.%s does not accept %s" target_type target_variant
                 (Types.to_string inner_error_type))
        | field_type :: rest -> (
            match
              Unify.unify
                (Types.canonicalize_mono_type inner_error_type)
  ...
  | _ -> failwith (Printf.sprintf "Codegen error: unknown wrap target %s.%s" target_type target_variant)
...
and emit_top_level_result_try_binding
  ...
  Printf.sprintf
    ...
    ind binding_name binding_go_type ind result_var result_expr ind result_var ind result_go_name ind binding_name
    success_payload ind result_go_name ind failure_value ind ind ind
...
and emit_top_level_option_try_binding
  ...
  Printf.sprintf
    "%svar %s %s\n%s%s := %s\n%sswitch %s.Tag {\n%scase %s_Some_tag:\n%s    %s = %s\n%scase %s_None_tag:\n%s    os.Exit(1)\n%sdefault:\n%s    panic(%S)\n%s}\n%s_ = %s\n"
    ind binding_name binding_go_type ind result_var result_expr ind result_var ind result_go_name ind binding_name
    some_payload ind result_go_name ind ind ind "unexpected Option tag in top-level try expression" ind ind
    binding_name
...
and emit_try_binding_for_context
  ...
  | Some _ ->
      if Option.is_some state.current_return_type then
        ...
        emit_top_level_result_try_binding state type_map env ~binding_name ~binding_type ~try_expr ~tried ~wrap
  | None -> (
...
and emit_wrap_expr
  ...
  Printf.sprintf
    ...
    (type_to_go state.mono expr_type)
    ...
    outer_result_go_name success_payload
    (indent_str { state with indent = state.indent + 1 })
    result_go_name
    (indent_str { state with indent = state.indent + 2 })
    outer_result_go_name failure_value
    (indent_str { state with indent = state.indent + 1 })
    (indent_str { state with indent = state.indent + 2 })
...
and emit_try_or_expr
    ...
    ~(try_expr : AST.expression)
    ~(tried : AST.expression)
    ~(fallback : AST.expression) : string =
  let expr_type = get_type type_map try_expr in
  let tried_type = Types.canonicalize_mono_type (get_type type_map tried) in
  let fallback_expr = emit_expr_for_expected_type state type_map env expr_type fallback in
  match result_type_parts_for_codegen tried_type with
  | Some (result_enum_name, result_type_args, _success_type, _error_type) ->
      track_enum_inst state.mono tried_type;
      let result_var = fresh_temp_name state "__try_result" in
      let result_expr = emit_expr_for_expected_type state type_map env tried_type tried in
      let result_go_name = mangle_type tried_type in
      ...
      Printf.sprintf
        ...
        (indent_str { state with indent = state.indent + 1 })
        (indent_str { state with indent = state.indent + 2 })
        "unexpected Result tag in try or expression"
        (indent_str { state with indent = state.indent + 1 })
        (indent_str state)
  | None -> (
      match option_type_parts_for_codegen tried_type with
      | Some (option_enum_name, option_type_args, _value_type) ->
          track_enum_inst state.mono tried_type;
          let result_var = fresh_temp_name state "__try_option" in
          let result_expr = emit_expr_for_expected_type state type_map env tried_type tried in
          let result_go_name = mangle_type tried_type in
          ...
          Printf.sprintf
            ...
            (indent_str { state with indent = state.indent + 1 })
            (indent_str { state with indent = state.indent + 2 })
            "unexpected Option tag in try or expression"
            (indent_str { state with indent = state.indent + 1 })
            (indent_str state)
      | None -> failwith "Codegen error: try or expression expected Result or Option tried type")
  ...
  let nested =
    ...
    | AST.Match (scrutinee, arms) ->
        ...
        @ List.concat_map
            ...
            arms
    | AST.Try { tried; fallback; _ } -> (
        collect_local_call_arg_types_expr name type_map env tried
        @
        match fallback with
        | None -> []
...
and emit_stmt
  ...
  | AST.Let let_binding -> (
      ...
      if let_binding.name = "_" then
        ...
        let expr_type =
          match
            explicit_binding_annotation_type ~type_bindings:state.current_type_bindings
            ...
            | None ->
                ...
                let binding_code =
                  Printf.sprintf "%svar %s %s\n%s%s = %s\n%s_ = %s\n" ind go_binding_name go_type ind
                    go_binding_name expr_str ind go_binding_name
                in
                (binding_code, env'))
        | AST.Try { tried; wrap; fallback = None } ->
            let binding_code =
...
let specialized_function_type_bindings
    (func_def : func_def) (inst : instantiation) (effective_return_type : Types.mono_type) :
    (string * Types.mono_type) list =
  let generic_names =
    match func_def.generics with
    | None -> []
    | Some generics -> List.map (fun (generic : AST.generic_param) -> generic.name) generics
  in
  match generic_names with
  | [] -> []
  | _ -> (
      let generic_seed = List.map (fun name -> (name, Types.TVar name)) generic_names in
      let convert_type_expr te = Annotation.type_expr_to_mono_type_with generic_seed te in
      let declared_param_types =
        List.map2
          (fun (_name, annot_opt) concrete_type ->
            match annot_opt with
            | Some type_expr -> (
                match convert_type_expr type_expr with
                | Ok typ -> typ
                | Error _ -> concrete_type)
            | None -> concrete_type)
          func_def.signature_params inst.concrete_types
      in
      let declared_return_type =
        match func_def.return_type with
        | Some type_expr -> (
            match convert_type_expr type_expr with
            | Ok typ -> typ
      ...
      | Ok subst ->
          generic_names
          |> List.map (fun name ->
                 let resolved = Types.apply_substitution subst (Types.TVar name) in
                 (name, resolved)))
...
let emit_specialized_func
  ...
  let function_type_bindings = specialized_function_type_bindings func_def inst effective_return_type in
               ...
               if v.fields = [] then
                 ...
                 let context_inits =
                   if is_error_enum then
                     ...
                     []
                 in
                 let inits = [ Printf.sprintf "Tag: %s_%s_tag" go_type_name v.name ] @ context_inits in
                 Printf.sprintf "func %s() %s {\n\treturn %s{%s}\n}\n" constructor_name go_type_name go_type_name
                   (String.concat ", " inits)
               else
                 ...
                 Printf.sprintf "func %s(%s) %s {\n\treturn %s{%s}\n}\n" constructor_name params go_type_name
...
let collect_inherent_call_sites
  ...
  let maybe_add_call_site
      (acc : inherent_call_site list)
      ~(method_name : string)
      ~(receiver_type : Types.mono_type)
  ...
  let rec collect_expr
      ...
      (acc : inherent_call_site list)
      (env : Infer.type_env)
      (expr : AST.expression) : inherent_call_site list =
    ...
    | AST.Hash pairs ->
        List.fold_left
          (fun acc' (k, v) ->
            let acc'' = collect_expr acc' env k in
            collect_expr acc'' env v)
          acc pairs
    | AST.Prefix (_, e) -> collect_expr acc env e
    | AST.Infix (l, _, r) -> collect_expr (collect_expr acc env l) env r
    | AST.TypeCheck (e, _type_ann) -> collect_expr acc env e
    | AST.If (condition, consequence, alternative) -> (
        let acc' = collect_expr acc env condition in
        let acc'', _ = collect_stmt acc' env consequence in
        match alternative with
        | None -> acc''
        | Some alt ->
            let acc''', _ = collect_stmt acc'' env alt in
            acc''')
    | AST.Function fn_expr ->
        let arity = List.length fn_expr.params in
        let param_types =
          match extract_param_types_exact arity (get_type type_map expr) with
          | Some (types, _) -> types
          | None -> []
        in
        let body_env = add_param_bindings env (List.map fst fn_expr.params) param_types in
        let acc', _ = collect_stmt acc body_env fn_expr.body in
        acc'
    | AST.Call (callee, args) ->
        let callee_type_opt =
          ...
          | AST.Identifier name -> (
              match Infer.TypeEnv.find_opt name env with
              | Some (Types.Forall (_, t)) -> Some t
              | None -> Hashtbl.find_opt type_map callee.id)
          | _ -> Hashtbl.find_opt type_map callee.id
        in
        let expected_arg_types =
          match callee_type_opt with
          | Some callee_type -> (
          ...
          | None -> []
        in
        let acc' = collect_expr acc env callee in
        let rec collect_args acc'' remaining_args remaining_expected =
          match (remaining_args, remaining_expected) with
          | [], _ -> acc''
          | arg :: rest_args, expected :: rest_expected ->
              collect_args (collect_expr ~expected_type:(Some expected) acc'' env arg) rest_args rest_expected
          | arg :: rest_args, [] -> collect_args (collect_expr acc'' env arg) rest_args []
        in
        collect_args acc' args expected_arg_types
    | AST.EnumConstructor (_enum_name, _variant_name, args) ->
        List.fold_left (fun acc' arg -> collect_expr acc' env arg) acc args
    | AST.Match (scrutinee, arms) ->
        let acc' = collect_expr acc env scrutinee in
        List.fold_left (fun acc' (arm : AST.match_arm) -> collect_expr acc' env arm.body) acc' arms
    | AST.Try { tried; fallback; _ } ->
        ...
        let acc' =
          List.fold_left
            (fun acc' (field : AST.record_field) ->
              ...
              | None -> acc'
              | Some v -> (
                  match expected_field_type field.field_name with
                  | Some field_type -> collect_expr ~expected_type:(Some field_type) acc' env v
                  | None -> collect_expr acc' env v))
            acc fields
        in
        match spread with
        | None -> acc'
        | Some s -> collect_expr acc' env s)
    | AST.FieldAccess (receiver, field_name) -> (
        match
          resolve_field_access_callable ~call_resolution_map ~method_type_args_map type_map env ~expected_type
            expr receiver field_name
        with
        | QualifiedInherentValue { method_name; receiver_type; method_type_args } ->
            maybe_add_call_site acc ~method_name ~receiver_type ~method_type_args
        | EnumConstructorValue _ | QualifiedTraitValue _ -> acc
        | NotQualifiedCallable -> collect_expr acc env receiver)
    | AST.MethodCall { mc_receiver = receiver; mc_method = method_name; mc_args = args; _ } -> (
  ...
  and collect_stmt
      ...
      (acc : inherent_call_site list)
      (env : Infer.type_env)
      (stmt : AST.statement) : inherent_call_site list * Infer.type_env =
    match stmt.stmt with
    | AST.Let let_binding ->
        let expr_type =
          match explicit_binding_annotation_type let_binding.type_annotation with
    ...
    | AST.TraitDef trait_def ->
        ( List.fold_left
            (fun acc' (m : AST.method_sig) ->
              match m.method_default_impl with
              | None -> acc'
              | Some e -> collect_expr acc' env e)
            acc trait_def.methods,
          env )
    | AST.ImplDef impl ->
        ( List.fold_left
            (fun acc' (m : AST.method_impl) ->
              let acc'', _ = collect_stmt acc' env m.impl_method_body in
              acc'')
    ...
    | AST.InherentImplDef impl ->
        ( List.fold_left
          ...
          env )
  and collect_stmt_list (acc : inherent_call_site list) (env : Infer.type_env) (stmts : AST.statement list) :
      inherent_call_site list =
    let rec go acc' env' = function
      | [] -> acc'
      | stmt :: rest ->
          let next_acc, next_env = collect_stmt ~following_stmts:rest acc' env' stmt in
          go next_acc next_env rest
    in
    go acc env stmts
  in
  collect_stmt_list [] typed_env program
  |> List.sort (fun a b ->
...
let emit_record_derived_impl
  ...
  | Typecheck.Trait_registry.DeriveOrd ->
      track_enum_inst state.mono (Types.TEnum ("Ordering", []));
      ...
      let body =
        ...
        | _ ->
            ...
            |> List.map (fun (less_expr, eq_expr) ->
                   Printf.sprintf "\tif %s { return %s }\n\tif !%s { return %s }\n" less_expr ordering_less_ctor
                     eq_expr ordering_greater_ctor)
            |> String.concat ""
      in
      Some
        (Printf.sprintf "func ord_compare_%s(x, y %s) %s {\n%s\treturn %s\n}\n" type_suffix type_str
           ordering_go_type_name body ordering_equal_ctor)
  | Typecheck.Trait_registry.DeriveHash ->
      List.iter
        (fun (f : Types.record_field_type) ->
          register_builtin_impl_use state.mono ~trait_name:"hash" ~method_name:"hash" ~for_type:f.typ)
        fields;
      let hash_steps =
        fields
...
let emit_named_wrapper_derived_impl
  ...
  match derive_kind with
  | Typecheck.Trait_registry.DeriveEq ->
      register_builtin_impl_use state.mono ~trait_name:"eq" ~method_name:"eq" ~for_type:representation;
      Some
  ...
  | Typecheck.Trait_registry.DeriveShow ->
      register_builtin_impl_use state.mono ~trait_name:"show" ~method_name:"show" ~for_type:representation;
      Some
  ...
  | Typecheck.Trait_registry.DeriveDebug ->
      register_builtin_impl_use state.mono ~trait_name:"debug" ~method_name:"debug" ~for_type:representation;
      Some
        (Printf.sprintf "func debug_debug_%s(x %s) string {\n\treturn debug_debug_%s(%s(x))\n}\n" type_suffix
           type_str rep_suffix rep_go_type)
  | Typecheck.Trait_registry.DeriveOrd ->
      track_enum_inst state.mono (Types.TEnum ("Ordering", []));
      register_builtin_impl_use state.mono ~trait_name:"ord" ~method_name:"compare" ~for_type:representation;
      Some
        (Printf.sprintf "func ord_compare_%s(x, y %s) %s {\n\treturn ord_compare_%s(%s(x), %s(y))\n}\n"
           type_suffix type_str ordering_go_type_name rep_suffix rep_go_type rep_go_type)
  | Typecheck.Trait_registry.DeriveHash ->
      register_builtin_impl_use state.mono ~trait_name:"hash" ~method_name:"hash" ~for_type:representation;
      Some
...
let emit_enum_derived_impl
  ...
  | Typecheck.Trait_registry.DeriveOrd ->
      ...
      Some
        (Printf.sprintf
           "func ord_compare_%s(x, y %s) %s {\n\tif x.Tag < y.Tag { return %s }\n\tif x.Tag > y.Tag { return %s }\n\treturn %s\n}\n"
           type_suffix type_str ordering_go_type_name ordering_less_ctor ordering_greater_ctor ordering_equal_ctor)
  | Typecheck.Trait_registry.DeriveHash ->
...
let emit_registry_derived_impls (state : emit_state) (program : AST.program) : string =
  ...
  in
  let requested_impls =
    DerivedImplUseSet.elements state.mono.derived_impl_uses
    |> List.filter_map (fun (trait_name, for_type) -> Typecheck.Trait_registry.lookup_impl trait_name for_type)
  in
  let emitted_impl_keys = ref StringPairSet.empty in
  let emit_impl (impl : Typecheck.Trait_registry.impl_def) : string option =
    if has_unresolved_codegen_type impl.impl_for_type then
      None
    else
      let trait_name = canonical_codegen_trait_name impl.impl_trait_name in
      let type_suffix = mangle_type impl.impl_for_type in
      let impl_key = (trait_name, type_suffix) in
      if StringPairSet.mem impl_key !emitted_impl_keys then
        None
      else (
        emitted_impl_keys := StringPairSet.add impl_key !emitted_impl_keys;
        if should_emit trait_name type_suffix then
          ...
          | Some derive_kind -> (
              ...
              | Types.TEnum _ -> emit_enum_derived_impl state derive_kind impl.impl_for_type
              | Types.TNamed (type_name, type_args) -> (
                  match named_type_codegen_body_exn type_name type_args with
...
let emit_inherent_methods
  ...
  let initial_call_sites =
    collect_inherent_call_sites ~concrete_only:state.mono.concrete_only state.mono.call_resolution_map
      state.mono.method_type_args_map state.mono.placeholder_rewrite_map type_map typed_env program
  in
  List.iter
    (fun (site : inherent_call_site) ->
      state.mono.inherent_call_uses <-
        InherentCallUseSet.add
          (site.call_method_name, site.call_receiver_type, site.call_method_type_args)
          state.mono.inherent_call_uses)
    initial_call_sites;
  ...
  let current_call_sites () =
    InherentCallUseSet.elements state.mono.inherent_call_uses
    |> List.map (fun (call_method_name, call_receiver_type, call_method_type_args) ->
           { call_method_name; call_receiver_type; call_method_type_args })
  in
  ...
  let emit_for_impl (impl : AST.inherent_impl_def) : string list =
    ...
    let relevant_call_sites =
      ...
      |> List.filter (fun site ->
             List.exists
               (fun (m : AST.method_impl) -> m.impl_method_name = site.call_method_name)
               impl.inherent_methods)
    in
                    ...
                    if has_concrete_target m.impl_method_name site.call_receiver_type then
                      ...
                      | Ok subst ->
                          ...
                          then
                            ...
                            None))
  in
  let rec emit_pending (acc : string list) =
    let newly_emitted = inherent_impls |> List.concat_map emit_for_impl in
    match newly_emitted with
    | [] -> String.concat "\n" (List.rev acc)
    | _ -> emit_pending (List.rev_append newly_emitted acc)
  in
  emit_pending []
...
let emit_builtin_impls (state : mono_state) (program : AST.program) : string =
  ...
  let all_builtins =
    [
      ...
      (("eq", "eq", "int64"), "func eq_eq_int64(x, y int64) bool {\n\treturn x == y\n}");
      (("eq", "eq", "bool"), "func eq_eq_bool(x, y bool) bool {\n\treturn x == y\n}");
      (("eq", "eq", "string"), "func eq_eq_string(x, y string) bool {\n\treturn x == y\n}");
      (("eq", "eq", "float64"), "func eq_eq_float64(x, y float64) bool {\n\treturn x == y\n}");
      ...
      (("hash", "hash", "int64"), "func hash_hash_int64(x int64) int64 {\n\treturn x\n}");
      (("hash", "hash", "bool"), "func hash_hash_bool(x bool) int64 {\n\tif x { return 1 } else { return 0 }\n}");
      ( ("hash", "hash", "string"),
        "func hash_hash_string(x string) int64 {\n\tvar h int64 = 0\n\tfor _, c := range x { h = h*31 + int64(c) }\n\treturn h\n}"
      );
      (* num trait implementations *)
      (("num", "add", "int64"), "func num_add_int64(x, y int64) int64 {\n\treturn x + y\n}");
      (("num", "sub", "int64"), "func num_sub_int64(x, y int64) int64 {\n\treturn x - y\n}");
      (("num", "mul", "int64"), "func num_mul_int64(x, y int64) int64 {\n\treturn x * y\n}");
      (("num", "div", "int64"), "func num_div_int64(x, y int64) int64 {\n\treturn x / y\n}");
      (("num", "add", "float64"), "func num_add_float64(x, y float64) float64 {\n\treturn x + y\n}");
      (("num", "sub", "float64"), "func num_sub_float64(x, y float64) float64 {\n\treturn x - y\n}");
      (("num", "mul", "float64"), "func num_mul_float64(x, y float64) float64 {\n\treturn x * y\n}");
      (("num", "div", "float64"), "func num_div_float64(x, y float64) float64 {\n\treturn x / y\n}");
      (* rem trait implementations *)
      (("rem", "rem", "int64"), "func rem_rem_int64(x, y int64) int64 {\n\treturn x % y\n}");
      (* neg trait implementations *)
      (("neg", "neg", "int64"), "func neg_neg_int64(x int64) int64 {\n\treturn -x\n}");
      (("neg", "neg", "float64"), "func neg_neg_float64(x float64) float64 {\n\treturn -x\n}");
    ]
  in

  let needed_impls =
    all_builtins
    |> List.filter (fun ((trait_name, method_name, type_name), _code) ->
           BuiltinImplUseSet.mem (trait_name, method_name, type_name) state.builtin_impl_uses
           && not (is_user_defined trait_name type_name))
  in
  ...
  let needs_strconv ((trait_name, method_name, type_name), _code) =
    ...
    | _ -> false
  in
  if List.exists needs_strconv needed_impls then
    add_go_import state "strconv";

  let impl_codes = List.map snd needed_impls in
  String.concat "\n\n" impl_codes

let emit_error_frames_conversion
    (state : mono_state) ~(context_expr : string) ~(frame_type : string) ~(return_expr : string) : string =
  ...
  |> List.filter_map (fun (method_name, for_type) ->
         ...
         | Types.TEnum (enum_name, _) when Typecheck.Enum_registry.is_error_enum enum_name ->
             ...
             let code =
               ...
               | _ ->
                   failwith (Printf.sprintf "Codegen error: unsupported synthetic error method '%s'" method_name)
             in
             Some code
         | _ -> None)
  |> String.concat "\n\n"
...
let rec abi_type_go_for_main ~(api_alias : string) (boundary : Typecheck.Shim_boundary.boundary_type) : string =
  ...
  | BFloat -> "float64"
  | BStr -> "string"
  | BStdOption inner -> Printf.sprintf "marmoset.Option[%s]" (abi_type_go_for_main ~api_alias inner)
  | BStdResult (ok_type, err_type) ->
      Printf.sprintf "marmoset.Result[%s, %s]"
        (abi_type_go_for_main ~api_alias ok_type)
        (abi_type_go_for_main ~api_alias err_type)
  | BList inner -> Printf.sprintf "[]%s" (abi_type_go_for_main ~api_alias inner)
  | BOwnerEnum enum ->
...
let rec track_boundary_marmoset_types (state : mono_state) (boundary : Typecheck.Shim_boundary.boundary_type) :
    unit =
  match boundary with
  | BUnit | BBool | BInt | BFloat | BStr | BStdBytes | BExternHandle _ -> ()
  | BStdOption inner ->
      track_enum_inst state (Typecheck.Shim_boundary.to_mono_type boundary);
      track_boundary_marmoset_types state inner
  | BStdResult (ok_type, err_type) ->
      track_enum_inst state (Typecheck.Shim_boundary.to_mono_type boundary);
      track_boundary_marmoset_types state ok_type;
      track_boundary_marmoset_types state err_type
  | BList inner -> track_boundary_marmoset_types state inner
  | BOwnerEnum enum ->
      track_enum_inst state (Typecheck.Shim_boundary.to_mono_type boundary);
...
let boundary_marmoset_go_type (state : mono_state) (boundary : Typecheck.Shim_boundary.boundary_type) : string =
  track_boundary_marmoset_types state boundary;
  type_to_go state (Typecheck.Shim_boundary.to_mono_type boundary)

let enum_def_exn (enum_name : string) : Typecheck.Enum_registry.enum_def =
  match Typecheck.Enum_registry.lookup enum_name with
  | Some enum_def -> enum_def
  | None -> failwith (Printf.sprintf "Codegen error: missing enum definition for shim boundary %s" enum_name)

let enum_field_boundary_exn ~(owner_module_id : string) (field_type : Types.mono_type) :
    Typecheck.Shim_boundary.boundary_type =
  match Typecheck.Shim_boundary.classify ~owner_module_id field_type with
  | Ok boundary -> boundary
  | Error diag ->
      failwith
        (Printf.sprintf "Codegen error: unsupported enum payload in shim boundary: %s"
           (Diagnostic.render_cli ~source_lookup:(fun _ -> None) diag))

let variant_payload_expr
    (state : mono_state)
    ~(enum_name : string)
    ~(type_args : Types.mono_type list)
    ~(variant_name : string)
    ~(position : int)
    ~(value_expr : string) : string =
  let enum_def = enum_def_exn enum_name in
  let layout = analyze_enum_layout state enum_def type_args in
  match List.assoc_opt variant_name layout.variant_maps with
  | None ->
      failwith
        (Printf.sprintf "Codegen error: missing enum layout for shim boundary variant %s.%s" enum_name
           variant_name)
  | Some mappings -> (
      match List.find_opt (fun (pos, _) -> pos = position) mappings with
      | None ->
          failwith
            (Printf.sprintf "Codegen error: missing enum payload layout for shim boundary variant %s.%s field %d"
               enum_name variant_name position)
      | Some (_, mapping) ->
          let field_expr = Printf.sprintf "%s.%s" value_expr mapping.data_field_name in
          if mapping.boxed then
            Printf.sprintf "(*%s)" field_expr
...
let rec to_abi_expr
    ...
    (expr : string) : string =
  match boundary with
  | BBool | BInt | BFloat | BStr -> expr
  | BUnit -> "marmoset.NewUnit()"
  | BStdOption inner ->
      let value_type = boundary_marmoset_go_type state boundary in
      let option_enum_name, option_type_args =
        match Typecheck.Shim_boundary.to_mono_type boundary with
        | Types.TEnum (name, args) -> (name, args)
        | _ -> failwith "Codegen error: Option shim boundary did not lower to an enum"
      in
      let inner_abi_type = abi_type_go_for_main ~api_alias inner in
      let inner_expr =
        let payload =
          variant_payload_expr state ~enum_name:option_enum_name ~type_args:option_type_args ~variant_name:"Some"
            ~position:0 ~value_expr:"__value"
        in
        to_abi_expr state ~api_alias ~owner_module_id ~context inner payload
      in
      Printf.sprintf
        "(func(__value %s) marmoset.Option[%s] {\n\t\tswitch __value.Tag {\n\t\tcase %s_Some_tag:\n\t\t\treturn marmoset.Some[%s](%s)\n\t\tcase %s_None_tag:\n\t\t\treturn marmoset.None[%s]()\n\t\tdefault:\n\t\t\tpanic(%S)\n\t\t}\n\t})(%s)"
        value_type inner_abi_type value_type inner_abi_type inner_expr value_type inner_abi_type
        (Printf.sprintf "%s: invalid Marmoset Option tag crossing shim boundary" context)
        expr
  | BStdResult (ok_type, err_type) ->
      let value_type = boundary_marmoset_go_type state boundary in
      let result_enum_name, result_type_args =
        match Typecheck.Shim_boundary.to_mono_type boundary with
        | Types.TEnum (name, args) -> (name, args)
        | _ -> failwith "Codegen error: Result shim boundary did not lower to an enum"
      in
      let ok_abi_type = abi_type_go_for_main ~api_alias ok_type in
      let err_abi_type = abi_type_go_for_main ~api_alias err_type in
      let ok_expr =
        let payload =
          variant_payload_expr state ~enum_name:result_enum_name ~type_args:result_type_args
            ~variant_name:"Success" ~position:0 ~value_expr:"__value"
        in
        to_abi_expr state ~api_alias ~owner_module_id ~context ok_type payload
      in
      let err_expr =
        let payload =
          variant_payload_expr state ~enum_name:result_enum_name ~type_args:result_type_args
            ~variant_name:"Failure" ~position:0 ~value_expr:"__value"
        in
        to_abi_expr state ~api_alias ~owner_module_id ~context err_type payload
      in
      Printf.sprintf
        "(func(__value %s) marmoset.Result[%s, %s] {\n\t\tswitch __value.Tag {\n\t\tcase %s_Success_tag:\n\t\t\treturn marmoset.Success[%s, %s](%s)\n\t\tcase %s_Failure_tag:\n\t\t\treturn marmoset.Failure[%s, %s](%s)\n\t\tdefault:\n\t\t\tpanic(%S)\n\t\t}\n\t})(%s)"
        value_type ok_abi_type err_abi_type value_type ok_abi_type err_abi_type ok_expr value_type ok_abi_type
        err_abi_type err_expr
        (Printf.sprintf "%s: invalid Marmoset Result tag crossing shim boundary" context)
        expr
  | BList inner ->
      let value_type = boundary_marmoset_go_type state boundary in
      let abi_type = abi_type_go_for_main ~api_alias boundary in
      let convert_item = to_abi_expr state ~api_alias ~owner_module_id ~context inner "__item" in
      Printf.sprintf
        "(func(__value %s) %s {\n\t\t__out := make(%s, len(__value))\n\t\tfor __i, __item := range __value {\n\t\t\t__out[__i] = %s\n\t\t}\n\t\treturn __out\n\t})(%s)"
        value_type abi_type abi_type convert_item expr
  | BOwnerEnum enum ->
      let value_type = boundary_marmoset_go_type state boundary in
      let api_type = abi_type_go_for_main ~api_alias boundary in
      let enum_def = enum_def_exn enum.enum_name in
      let type_args = List.map Typecheck.Shim_boundary.to_mono_type enum.enum_type_args in
      let subst = Types.substitution_of_list (List.combine enum_def.type_params type_args) in
      let api_base = exported_go_ident (Typecheck.Display_names.display_binding_name enum.enum_name) in
      let cases =
        enum_def.variants
        |> List.map (fun (variant : Typecheck.Enum_registry.variant_def) ->
               let api_variant_type =
                 Printf.sprintf "%s.%s%s" api_alias api_base (exported_go_ident variant.name)
               in
               let fields =
                 variant.fields
                 |> List.mapi (fun idx field_type ->
                        ...
                        let payload =
                          variant_payload_expr state ~enum_name:enum.enum_name ~type_args
                            ~variant_name:variant.name ~position:idx ~value_expr:"__value"
                        in
                        Printf.sprintf "Field%d: %s" idx
                          (to_abi_expr state ~api_alias ~owner_module_id ~context field_boundary payload))
                 |> String.concat ", "
               in
               let constructor =
                 if fields = "" then
                   ...
                   Printf.sprintf "%s{%s}" api_variant_type fields
               in
               Printf.sprintf "\t\tcase %s_%s_tag:\n\t\t\treturn %s" value_type variant.name constructor)
        |> String.concat "\n"
      in
      Printf.sprintf
        "(func(__value %s) %s {\n\t\tswitch __value.Tag {\n%s\n\t\tdefault:\n\t\t\tpanic(%S)\n\t\t}\n\t})(%s)"
  ...
  | BNamedWrapper wrapper -> (
      ...
      match wrapper.wrapper_payloads with
      | [ payload_boundary ] ->
          let representation_go_type = type_to_go state (Typecheck.Shim_boundary.to_mono_type payload_boundary) in
          let payload_expr = Printf.sprintf "%s(__value)" representation_go_type in
          let converted = to_abi_expr state ~api_alias ~owner_module_id ~context payload_boundary payload_expr in
          Printf.sprintf "(func(__value %s) %s {\n\t\treturn %s\n\t})(%s)" value_type api_type converted expr
      | _ ->
          let fields =
            ...
            |> String.concat ", "
          in
          Printf.sprintf "(func(__value %s) %s {\n\t\treturn %s{%s}\n\t})(%s)" value_type api_type api_type fields
            expr)
  | BExternHandle _ ->
      let value_type = boundary_marmoset_go_type state boundary in
      let api_type = abi_type_go_for_main ~api_alias boundary in
...
and from_abi_expr
  ...
  | BOwnerEnum enum ->
      ...
      let cases =
        ...
        |> List.map (fun (variant : Typecheck.Enum_registry.variant_def) ->
               ...
               let constructed =
                 maybe_attach_shim_error_context state ?error_frame ~enum_name:enum.enum_name ~variant ~value_type
                   constructed
               in
               Printf.sprintf "\t\tcase %s:\n\t\t\t_ = __variant\n\t\t\treturn %s" api_variant_type constructed)
        |> String.concat "\n"
      in
      Printf.sprintf
        "(func(__value %s) %s {\n\t\tif __value == nil {\n\t\t\tpanic(%S)\n\t\t}\n\t\tswitch __variant := __value.(type) {\n%s\n\t\tdefault:\n\t\t\tpanic(%S)\n\t\t}\n\t})(%s)"
        api_type value_type
        (Printf.sprintf "%s: nil enum returned by shim" context)
        cases
  ...
  | BNamedWrapper wrapper -> (
      ...
      | [ payload_boundary ] ->
          ...
          in
          Printf.sprintf "(func(__value %s) %s {\n\t\treturn %s(%s)\n\t})(%s)" api_type value_type value_type
            payload_expr expr
      | _ ->
          let payload_exprs =
            wrapper.wrapper_payloads
...
let emit_extern_wrapper (state : mono_state) (func : Typecheck.Resolution_artifacts.extern_func) : string =
  ...
  let api_alias = api_package_name func.shim_id in
  ...
  let params =
    ...
    |> List.map (fun (name, boundary) ->
           Printf.sprintf "%s %s" (go_safe_ident name) (boundary_marmoset_go_type state boundary))
    |> String.concat ", "
  in
  let return_type = boundary_marmoset_go_type state func.return_boundary_type in
  let call_args =
    List.combine func.param_names func.param_boundary_types
    |> List.map (fun (name, boundary) ->
           to_abi_expr state ~api_alias ~owner_module_id:func.owner_module_id ~context boundary
             (go_safe_ident name))
    |> String.concat ", "
  in
  let body =
...
let emit_program_with_typed_env
  ...
  ignore
    (List.fold_left
       (fun bindings stmt -> collect_funcs_stmt ~top_level:true ~available_bindings:bindings mono_state stmt)
       StringSet.empty program);

  (* Pass 2: Collect instantiations using the already-typed environment and type_map *)
  ignore (collect_insts_stmt_list mono_state type_map typed_env program);
  ...
  (* Emit main body before specialized functions so concrete function-value uses
...
let normalize_aux_go_rel_path (rel_path : string) : (string, string) result =
  if String.equal rel_path "" then
    ...
    if List.exists (fun part -> String.equal part "" || String.equal part "." || String.equal part "..") parts
    then
...
let find_existing_path_upwards ?toolchain_root (parts : string list) : string option =
  let root_candidate =
    match toolchain_root with
    | Some root ->
        let path = concat_path_parts root parts in
...
let runtime_go_shim_dir ?toolchain_root (shim_id : string) : string option =
  find_existing_path_upwards ?toolchain_root [ "runtime"; "go"; "shims"; shim_id ]

let rec collect_regular_files ~(root : string) ~(rel_dir : string) : string list =
  let dir =
    if rel_dir = "" then
      root
    else
      Filename.concat root rel_dir
  in
  Sys.readdir dir
  |> Array.to_list
  |> List.sort String.compare
  |> List.concat_map (fun name ->
         let rel_path =
           if rel_dir = "" then
             ...
             Filename.concat rel_dir name
         in
         let path = Filename.concat root rel_path in
         if Sys.is_directory path then
           collect_regular_files ~root ~rel_dir:rel_path
         else
           [ rel_path ])

let shim_package_files ?toolchain_root (shim_id : string) : go_aux_file list =
  let source_root =
    match runtime_go_shim_dir ?toolchain_root shim_id with
  ...
  | files ->
      ...
      |> List.map (fun rel ->
             {
               rel_path = Filename.concat (Filename.concat "shims" shim_id) rel;
               contents = Option.value (read_file_if_exists (Filename.concat source_root rel)) ~default:"";
             })
...
let rec collect_boundary_handles
    (acc : Typecheck.Shim_boundary.extern_type_identity list) (boundary : Typecheck.Shim_boundary.boundary_type) :
...
let compare_boundary_wrapper
  ...
  | 0 ->
      String.compare
        (Typecheck.Shim_boundary.to_string (BNamedWrapper a))
...
let close_api_enum_names ~(owner_module_id : string) (enum_names : string list) : string list =
  let dependencies_of_enum acc enum_name =
    ...
    | Some enum_def ->
        enum_def.variants
        |> List.fold_left
             (fun acc (variant : Typecheck.Enum_registry.variant_def) ->
               variant.fields
               |> List.fold_left
                    (fun acc field_type ->
                      match enum_field_boundary ~owner_module_id field_type with
                      | None -> acc
                      | Some boundary -> collect_boundary_enums acc boundary)
                    acc)
             acc
  in
  let rec close seen pending =
    match pending with
    | [] -> List.sort_uniq String.compare seen
    | enum_name :: rest ->
        if List.mem enum_name seen then
          close seen rest
        else
          let deps = dependencies_of_enum [] enum_name in
          close (enum_name :: seen) (deps @ rest)
  in
  close [] enum_names
...
let emit_api_package (shim_id : string) (funcs : Typecheck.Resolution_artifacts.extern_func list) : go_aux_file =
  let handles =
    funcs
    |> List.concat_map collect_func_boundary_handles
    |> List.sort_uniq (fun (a : Typecheck.Shim_boundary.extern_type_identity) b ->
           match String.compare a.extern_type_name b.extern_type_name with
           | 0 -> String.compare a.owner_module_id b.owner_module_id
           | cmp -> cmp)
  ...
  let enum_names =
    funcs
    |> List.concat_map collect_func_boundary_enums
    |> List.sort_uniq String.compare
...
let shim_aux_go_files
  ...
  | [] -> []
  | _ ->
      let grouped = group_funcs_by_shim_id funcs in
      let api_files = grouped |> List.map (fun (shim_id, funcs) -> emit_api_package shim_id funcs) in
      let shim_files =
        grouped |> List.concat_map (fun (shim_id, _) -> shim_package_files ?toolchain_root shim_id)
      in
      validate_go_aux_files (marmoset_support_files ?toolchain_root () @ api_files @ shim_files)
  ...
  | Ok program -> (
      ...
      | Ok
          ...
          try
            ...
            let aux_go_files =
              if
                shim_aux_files = [] && String_utils.contains_substring ~needle:"\"marmoset_out/marmoset\"" main_go
              then
                marmoset_support_files ()
              else
                shim_aux_files
            in
            let output = { go_mod = default_go_mod; main_go; aux_go_files; diagnostics } in
            match validate_go_aux_files output.aux_go_files with
...
let%test "shim build emits marmoset support and api package for called std shim" =
  match
    compile_to_build ~file_id:"std.bytes"
      {|
extern "std/bytes" = {
  fn from_str(input: Str) -> Str
}
...
|}
  ...
  | Ok output ->
      ...
      List.mem "api/std/bytes/api.go" paths
...
let%test "generated shim api package includes owner enum variants" =
  ...
  let enum_boundary =
    Typecheck.Shim_boundary.BOwnerEnum { enum_name = "std__bytes__Status"; enum_type_args = [] }
  in
  Hashtbl.add extern_declarations shim_key
    {
      Typecheck.Resolution_artifacts.shim_key;
      shim_id = "std/bytes";
      owner_module_id = "std.bytes";
      source_qualifier = "bytes";
  ...
  match find_aux_file (shim_aux_go_files ~extern_declarations ~extern_calls ()) "api/std/bytes/api.go" with
  | None -> false
  | Some file ->
      string_contains file.contents "package mapi_std_bytes"
...
let register_std_bytes_decode_error_for_test () =
  Typecheck.Enum_registry.register
    {
  ...
  Hashtbl.add extern_declarations shim_key
    {
      ...
      owner_module_id = "std.bytes";
      source_qualifier = "bytes";
      marmoset_func_name = "to_str";
      go_symbol_name = "ToStr";
      param_names = [ "input" ];
  ...
  Hashtbl.add extern_declarations shim_key
    {
      ...
      owner_module_id = "std.bytes";
      source_qualifier = "bytes";
      marmoset_func_name = "from_str";
      go_symbol_name = "FromStr";
      param_names = [ "input" ];
      param_boundary_types = [ Typecheck.Shim_boundary.BStr ];
      return_boundary_type = Typecheck.Shim_boundary.BStr;
      is_effectful = false;
      source_span = Diagnostic.NoSpan;
      boundary_spans = [ Diagnostic.NoSpan; Diagnostic.NoSpan ];
    };
  add_std_bytes_to_str_decl_for_test extern_declarations;
  Hashtbl.add extern_calls 1
    {
      Typecheck.Resolution_artifacts.call_func_key = shim_key;
      call_arg_boundary_types = [ Typecheck.Shim_boundary.BStr ];
      call_return_boundary_type = Typecheck.Shim_boundary.BStr;
      call_effectful = false;
    };
  let output =
...
let%test "generated shim api package names are keyword safe" =
  ...
  let extern_declarations = Hashtbl.create 1 in
  let extern_calls = Hashtbl.create 1 in
  let enum_boundary = Typecheck.Shim_boundary.BOwnerEnum { enum_name = "std__bytes__Map"; enum_type_args = [] } in
  Hashtbl.add extern_declarations shim_key
    {
      Typecheck.Resolution_artifacts.shim_key;
      shim_id = "std/bytes";
  ...
  | Error _ -> false
  | Ok files -> (
      match find_aux_file output.aux_go_files "api/std/bytes/api.go" with
      | None -> false
      | Some file ->
          string_contains file.contents "package mapi_std_bytes"
...
let%test "std shim adapter imports and calls copied shim package" =
  match
    compile_to_build ~file_id:"std.bytes"
      {|
extern "std/bytes" = {
...
|}
  ...
  | Error _ -> false
  | Ok output ->
      string_contains output.main_go {|mshim_std_bytes "marmoset_out/shims/std/bytes"|}
      && string_contains output.main_go "mshim_std_bytes.FromStr(input)"
      && string_not_contains output.main_go "shim adapter not implemented"
      && Option.is_some (find_aux_file output.aux_go_files "shims/std/bytes/bytes.go")
...
let%test "non-capturing placeholder section argument hoists to helper" =
  match
    compile_string ~file_id:"<codegen>" "fn apply(x: Int, f: (Int) -> Int) -> Int = f(x)\napply(41, _ + 1)"
  with
  | Ok (code, _) -> string_contains code "func __section_" && not (string_contains code "apply(int64(41), func(")
  | Error _ -> false

let%test "capturing placeholder section argument stays inline" =
  match
    compile_string ~file_id:"<codegen>"
      "let delta = 1\nfn apply(x: Int, f: (Int) -> Int) -> Int = f(x)\napply(41, _ + delta)"
  with
  | Ok (code, _) -> string_contains code "func(it int64) int64"
  | Error _ -> false

let%test "projection placeholder section argument hoists to helper" =
  match
    compile_string ~file_id:"<codegen>"
      "type Post = { updated_at: Int }\nfn project(post: Post, f: (Post) -> Int) -> Int = f(post)\nlet post: Post = { updated_at: 7 }\nproject(post, _.updated_at)"
  with
  | Ok (code, _) -> string_contains code "func __section_" && not (string_contains code "project(post, func(")
  | Error _ -> false
...
let%test "generic empty array return uses function return type during build" =
  ...
  with
  | Ok out -> string_contains out.main_go "return []int64{}" && string_contains out.main_go "return []string{}"
  | Error _ -> false
...
let%test "emit if inside function body" =
  match
    compile_string ~file_id:"<codegen>" "fn abs(x: Int) -> Int = { if (x < 0) { -x } else { x } }\nabs(5)"
  with
  | Ok (code, _) ->
...
let%test "if statement without else in function followed by return" =
  match
    compile_string ~file_id:"<codegen>"
      "fn fib(n: Int) -> Int = { if (n < 2) { return n }; return n + 1 }\nfib(5)"
  with
  | Ok (code, _) ->
...
let%test "function returning wrapped scalar record compiles" =
  match
    compile_string ~file_id:"<codegen>" "fn mk(i: Int) -> { inner: Int } = { inner: i }\nlet o = mk(1)\no.inner"
  with
  | Ok (code, _) ->
...
let%test "emitter lowers inherent methods to inherent_* helpers" =
  match
    compile_string ~file_id:"<codegen>"
      "impl Int = { fn double(x: Int) -> Int = x + x }\nlet v = 2\nInt.double(v)"
  with
  | Ok (code, _) ->
...
|}
  ...
  match compile_string ~file_id:"<codegen>" source with
  | Ok (code, _) ->
      string_contains code "func inherent_tag_Result_int64_string(" && string_contains code "return \"concrete\""
  | Error _ -> false
...
let%test "type-check if in tail position emits without IIFE" =
  match
    compile_string ~file_id:"<codegen>" "fn f(x: Int | Str) -> Int = { if (x is Int) { x + 1 } else { 0 } }\nf(1)"
  with
  | Ok (code, _) ->
      (not (string_contains code "func() int64")) && string_contains code "switch x_typed := x.(type)"
  | Error _ -> false

let%test "statically narrowed local type-check avoids runtime type switch" =
  match
    compile_string ~file_id:"<codegen>"
      {|fn outer(x: Int | Str) -> Int = {
  if (x is Int) {
    let inner = (y: Int | Bool) -> if (y is Int) { y } else { 0 };
    inner(x)
  } else {
    0
  }
...
puts(outer(42))|}
  with
  | Ok (code, _) -> string_contains code "(y int64) int64 {\n        return y\n}"
  | Error _ -> false
...
let%test "create_mono_state preserves supplied trait object coercion map" =
  ...
  Hashtbl.replace coercions 9
    Typecheck.Resolution_artifacts.{ target_traits = [ "Eq"; "Show" ]; source_type = Types.TInt };
...
let%test "type_check_static_outcome recognizes already narrowed concrete types" =
  match type_check_static_outcome (Some Types.TInt) Types.TInt with
  | `Always_true -> true
  | _ -> false

let%test "type_check_static_outcome recognizes impossible concrete checks" =
  match type_check_static_outcome (Some Types.TString) Types.TInt with
  | `Always_false -> true
  | _ -> false
...
let%test "field-path type-check if emits a runtime type switch" =
  ...
  | Ok (go_code, _) ->
      string_contains go_code "box_value_subject :="
      && string_contains go_code "switch box_value_typed := box_value_subject.(type)"
      && string_contains go_code "box_value_complement := box_value_subject.(string)"
  | Error _ -> false
...
let%test "Dyn codegen emits derived helpers for generic default trait boxing" =
  Fun.protect
    ~finally:(fun () ->
      Typecheck.Type_registry.clear ();
      Typecheck.Trait_registry.clear ();
...
|}
      ...
      match compile_string ~file_id:"<codegen>" source with
      | Ok (code, _) ->
          string_contains code "func show_show_record_value_int64_closed(x Record_value_int64) string"
          && string_contains code "show_show_record_value_int64_closed(__receiver.(Record_value_int64))"
      | Error _ -> false)
...
let%test "structured imports ignore package-looking string literals" =
  ...
  | Ok (code, _) ->
      (not (string_contains code {|import "fmt"|})) && not (string_contains code {|import "strconv"|})
  | Error _ -> false
...
let%test "std shim call emits typed adapter wrapper and wrapper call" =
  match
    compile_string ~file_id:"std.bytes"
      {|
extern "std/bytes" = {
  fn from_str(input: Str) -> Str
}
...
|}
  ...
  | Ok (code, _) ->
      ...
      && string_contains code "func extern__std_bytes__from_str(input string) string"
      && string_contains code {|mshim_std_bytes "marmoset_out/shims/std/bytes"|}
      && string_contains code "mshim_std_bytes.FromStr(input)"
      && string_not_contains code "shim adapter not implemented"
      && string_contains code {|extern__std_bytes__from_str("ok")|}
  | Error _ -> false
...
|}
  with
  | Ok (code, _) ->
      (not (string_contains code "mext_")) && not (string_contains code "extern__std_bytes__from_str")
  | Error _ -> false

let%test "effectful unit shim wrapper emits unit-shaped adapter" =
  match
    compile_string ~file_id:"std.bytes"
      {|
extern "std/bytes" = {
...
|}
  ...
  | Ok (code, _) ->
      (not (string_contains code "mext_"))
      && string_contains code "func extern__std_bytes__println(s string) struct{}"
      && string_contains code {|mshim_std_bytes "marmoset_out/shims/std/bytes"|}
      && string_contains code "mshim_std_bytes.Println(s)"
      && string_contains code "return struct{}{}"
      && string_not_contains code "shim adapter not implemented"
      && string_contains code {|extern__std_bytes__println("ok")|}
  | Error _ -> false
...
|} in
  match compile_string ~file_id:"<codegen>" source with
  | Ok (code, _) ->
      string_contains code "get_x_record_x_int64_y_int64_closed" && not (string_contains code "apply(get_x,")
  | Error _ -> false
...
let%test "fingerprint_types distinguishes pure and effectful callables" =
  fingerprint_types [ Types.tfun Types.TInt Types.TInt ]
  <> fingerprint_types [ Types.tfun_eff Types.TInt Types.TInt ]

let%test "fingerprint_types distinguishes different unions" =
  fingerprint_types [ Types.normalize_union [ Types.TInt; Types.TBool ] ]
  <> fingerprint_types [ Types.normalize_union [ Types.TInt; Types.TString ] ]

let%test "instantiation identity includes return type" =
  let mk return_type =
    {
      func_name = "id";
      module_path = "main";
...
let%test "add_instantiation keeps return-only generic instantiations distinct" =
  let state = create_mono_state () in
  let mk return_type =
    {
      func_name = "id";
  ...
  InstSet.cardinal state.instantiations = 2
  && String.equal (mangle_instantiated_func_name state.instantiations (mk Types.TInt)) "id_int64_ret_int64"
  && String.equal (mangle_instantiated_func_name state.instantiations (mk Types.TBool)) "id_int64_ret_bool"
      ...
      | Ok program -> (
          ...
          | Ok
              ...
              ignore
                (List.fold_left
                   (fun bindings stmt ->
                     collect_funcs_stmt ~top_level:true ~available_bindings:bindings mono_state stmt)
                   StringSet.empty program);
              ignore (collect_insts_stmt_list mono_state type_map typed_env program);
...
puts(Show.show([1, 2]))|}
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      match compile_string ~file_id:"<codegen>" source with
      | Ok (code, _) ->
          string_contains code "func show_show_arr_int64(" && string_not_contains code "func Show_show_arr_int64("
      | Error _ -> false)

let%test "unused stdlib helpers are omitted from minimal program" =
  ...
  | Ok (code, _) ->
      ...
      && string_not_contains code "type Ordering struct"
      && string_not_contains code "func show_show_int64("
      && string_not_contains code "func eq_eq_int64("
      && string_not_contains code "func ord_compare_int64("
      && string_not_contains code "func num_add_int64("
      && string_not_contains code "func rem_rem_int64("
      && string_not_contains code "func neg_neg_int64("
  | Error _ -> false

let%test "explicit primitive trait calls emit only referenced builtin helpers" =
  match compile_string ~file_id:"<codegen>" "puts(Show.show(42))\nputs(Eq.eq(1, 1))\nputs(Num.add(1, 2))" with
  | Ok (code, _) ->
      count_occurrences code "func show_show_int64(" = 1
      && count_occurrences code "func eq_eq_int64(" = 1
      && count_occurrences code "func num_add_int64(" = 1
      && string_not_contains code "func num_sub_int64("
      && string_not_contains code "func num_mul_int64("
...
let%test "match with nested if avoids IIFE" =
  match
    compile_string ~file_id:"<codegen>" {|match 1 {
...
puts(print(1))|}
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      | Error diags ->
          List.iter
            (fun (diag : Diagnostic.t) -> Printf.printf "Diagnostic: [%s] %s\n" diag.code diag.message)
            diags;
          false)
...
let%test "successful build emits no missing-type warning text" =
  let result, stderr_output =
    capture_stderr (fun () -> compile_string ~file_id:"<codegen>" {|fn f(x: Int) -> Int = x + 1
puts(f(1))|})
  in
  match result with
