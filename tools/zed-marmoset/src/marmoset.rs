...
fn repo_dev_binary_candidates(manifest_dir: &Path) -> [String; 2] {
...
mod tests {
    ...
    #[test]
    fn prefers_repo_build_main_exe_when_launch_probe_accepts_it() {
        let manifest_dir = manifest_dir();
        let [main_exe, _install_bin] = repo_dev_binary_candidates(&manifest_dir);
        ...
        assert_eq!(probed, vec![main_exe]);
    }

    #[test]
    fn falls_back_to_installed_build_path_when_launch_probe_accepts_it() {
        let manifest_dir = manifest_dir();
        let [main_exe, install_bin] = repo_dev_binary_candidates(&manifest_dir);
        let mut probed = Vec::new();
        ...
        assert_eq!(probed, vec![main_exe, install_bin]);
    }
    ...
    fn does_not_depend_on_local_fs_visibility_for_repo_candidate() {
        ...
        let selected = repo_dev_binary(manifest_dir.as_path(), |path| path == main_exe);

        assert_eq!(selected, Some(main_exe));
    }
}
