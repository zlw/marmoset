...
Keywords (`let`, `fn`, `if`, `match`, `case`, `enum`, `shape`, `trait`, `impl`, `derive`, `type`, `return`, `try`, `wrap`, `or`, `override`, `is`), canonical builtin types (`Int`, `Str`, `Bool`, `Float`, `Unit`, `List`, `Map`), builtin functions (`puts`, `len`, `first`, `rest`, `push`), literals, operators, enum variants, shape fields, wrapper constructors, canonical `type` sums, type parameters, generic constraints, and more.
