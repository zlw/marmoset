...
- 2026-03-30: Kept syntax-highlighted hover code blocks, but wrapped bare type-only payloads as valid transparent type declarations (`type Hover = TypeExpr`) so editor hover highlighting can parse generic types like `Map[Str, Str]` instead of partially coloring them.
