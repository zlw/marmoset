...
- 2026-05-13 18:34 CEST: E3 commit created for shim-boundary error context injection, stable generated frame file ids, stdlib-shim decode-error coverage, and the updated FFI Go tree snapshot.
- 2026-05-13 18:36 CEST: E4 started. Adding RED coverage for `try` same-error propagation, `try ... wrap ...` typed wrapping, and rejection diagnostics before parser/typechecker/codegen changes.
- 2026-05-13 18:37 CEST: E4 RED observed with `dune runtest lib/frontend/syntax`: new parser/lowering tests fail because `Try`/`Wrap` tokens and `SETry`/`AST.Try` expression forms do not exist yet.
- 2026-05-13 19:00 CEST: E4 focused compiler checks are green: `dune runtest lib/frontend/syntax`, `dune runtest lib/frontend/typecheck`, `dune runtest lib/backend/go`, and `make integration result` pass after adding typed `try` propagation, `try ... wrap ...`, nested-wrap coverage, soft `wrap` parsing, and statement-level early-return codegen.
- 2026-05-13 19:00 CEST: E4 editor parser coverage is green under `npm run generate` and `npm test` in `tools/tree-sitter-marmoset`; TextMate and Neovim highlight queries now include `try`/`wrap` and wrap targets.
- 2026-05-13 19:05 CEST: E4 focused Dune gate was re-run after cleanup and is green under `dune runtest lib/frontend/syntax lib/frontend/typecheck lib/backend/go`.
- 2026-05-13 19:06 CEST: E4 full gate is green: `make integration result codegen_mono stdlib-shims`, `dune runtest tools/lsp/lib`, `test/ci/tree-sitter.sh`, `test/ci/editor-vscode.sh`, `test/ci/editor-nvim.sh`, `test/ci/editor-jetbrains.sh`, `cargo fmt --check --manifest-path tools/zed-marmoset/Cargo.toml`, `cargo test --locked --manifest-path tools/zed-marmoset/Cargo.toml`, `git diff --check`, and `git diff --cached --check` pass for `try` propagation, wrap targets, LSP/editor coverage, and tree-sitter generated output.
...
- 2026-05-13 21:27 CEST: E5 stdlib idiom follow-up started. Adding RED style coverage so `std.file` demonstrates `try ... wrap` for typed error adaptation and handle-open wrapping, while generic `std.io.Write.print` uses the minimal `Result` API instead of manual success/failure propagation.
- 2026-05-13 21:27 CEST: RED observed with `./test/ci/std-style.sh`: `std.file` still lacks `try ... wrap` at the decode adapter/open wrapper sites and `std.io.Write.print` still contains a manual `Result.Failure` propagation arm.
- 2026-05-13 21:30 CEST: RED observed for generic `try ... wrap` codegen: `make integration test/fixtures/result/r014_try_wrap_generic_success_type_codegen.mr` fails with unresolved type variable `t0` in `mangle_type`, and the stdlib-shims suite hits the same issue through `std.file.decode_bytes`.
- 2026-05-13 21:44 CEST: E5 stdlib idiom follow-up is green. `try` let-bindings now stay monomorphic, expected-type substitutions update expression type maps across constructor arguments and earlier block statements, enum emission tracks nested payload enum types, and `std.file`/`std.io` use the intended `try ... wrap` and minimal `Result` idioms. Verification passed with `./test/ci/std-style.sh`, `MARMOSET_ROOT=$PWD dune exec -- bin/main.exe check std/file.mr`, `MARMOSET_ROOT=$PWD dune exec -- bin/main.exe check std/io.mr`, `make integration prelude result stdlib-shims`, `dune runtest lib/frontend/typecheck lib/backend/go`, and `git diff --check`. `./test/ci/quality.sh` still reports pre-existing formatter drift in unrelated OCaml/LSP files, plus the touched `infer.ml` formatting was adjusted locally.
- 2026-05-13 21:45 CEST: E5 stdlib idiom follow-up commit created.
- 2026-05-13 22:14 CEST: Follow-up bare `wrap` expression slice started. Adding RED coverage for `result wrap Error.Variant` as the non-propagating sibling of `try result wrap Error.Variant`, with LSP traversal and stdlib usage updates in scope.
