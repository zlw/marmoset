...
- 2026-05-12 22:45 CEST: Fixed the Zed local-dev launcher path that could still pick the stale PATH-installed `marmoset` when the parent checkout directory was opened as the worktree. The stale binary reproduced the `unsupported shim boundary type std__path__Path` diagnostic, while the repo `./marmoset` accepted `std/path.mr`; the launcher now probes nested `marmoset/marmoset` repo binaries before falling back to PATH. Verification passed with `cargo test --locked` in `tools/zed-marmoset`; `test/ci/editor-zed.sh` reached the Rust tests and then stopped on local Python 3.9 lacking `tomllib`.
