...
- 2026-04-28 17:52 CEST: F1d module/import integration started; scope is extern signature type rewriting, qualifier collision diagnostics, and project artifact merging checks.
- 2026-04-28 18:00 CEST: F1d module/import integration green; extern signature type annotations rewrite through imports, qualifier collisions against namespaces/direct bindings/top-level/implicit core names are diagnosed, externs remain module-local, project extern artifacts merge, and `dune runtest lib/frontend`, `dune build @all`, and `make unit` pass.
- 2026-04-28 18:02 CEST: F2a structured Go import builder started; first slice preserves existing builtin import output before extern codegen is added.
- 2026-04-28 18:03 CEST: F2a structured Go import builder green; generated imports now come from explicit emission-site registration, string literals no longer trigger package imports, and `dune runtest lib/backend/go`, `dune build @all`, and `make unit` pass.
- 2026-04-28 18:04 CEST: F2b extern type-to-Go lowering green; codegen now has a v1 scalar-only extern boundary with backend rejection coverage for unsupported types, and `dune runtest lib/backend/go` plus `dune build @all` pass.
- 2026-04-28 18:05 CEST: F2c/F2d wrapper emission and extern call lowering green; used externs emit deterministic aliased imports and wrappers, unused declarations emit nothing, Unit-return wrappers sequence the Go call before returning Marmoset unit, duplicate basename packages stay distinct, and `dune runtest lib/backend/go`, `dune build @all`, and `make unit` pass.
- 2026-04-28 18:06 CEST: F3 module wrapper flow green at compiler level; wrapper modules expose ordinary exported APIs, extern qualifiers remain private, matching declarations across wrapper modules dedupe generated wrappers/imports, conflicting signatures across modules are rejected, and `dune runtest lib/frontend`, `dune build @all`, and `make unit` pass.
- 2026-04-28 18:07 CEST: F4 docs and integration harness implemented; `docs/features/ffi.md`, architecture/roadmap/index updates, `test/fixtures/ffi`, exact Go snapshot coverage, and `test/integration/13_ffi.sh` selector wiring are in place.
- 2026-04-28 18:08 CEST: FFI milestone complete and moved to done; `make unit`, `make integration ffi`, `make integration snapshots`, and focused `test/fixtures/ffi/*.mr` fixture runs pass.
- 2026-04-28 18:24 CEST: Codex-only hardening pass expanded exact generated-Go snapshots for unused externs, import sorting, duplicate basenames, and sanitized path collisions; broadened unsupported boundary type and qualifier shadowing coverage; and fixed a generated Go collision where an extern wrapper could redeclare a used zero-arg Marmoset function name.
